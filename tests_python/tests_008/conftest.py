...
def client(sandbox: Sandbox) -> Iterator[Client]:
    """One node with protocol 008.

    Activate protocol 008 one year in the past. This avoids waiting
...
def client_regtest_bis(sandbox: Sandbox) -> Iterator[Client]:
    """One node with protocol 008, regression test enabled.

    Activate protocol 008 one year in the past. (see fixture client).
    """
...
def clients(sandbox: Sandbox, request) -> Iterator[List[Client]]:
    """N node with protocol 008. Parameterized by the number of nodes.
