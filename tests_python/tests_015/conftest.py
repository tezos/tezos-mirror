...
def client(sandbox: Sandbox) -> Iterator[Client]:
    """One node with protocol 015.

    Activate protocol 015 one year in the past. This avoids waiting
...
def client_regtest_bis(sandbox: Sandbox) -> Iterator[Client]:
    """One node with protocol 015, regression test enabled.

    Activate protocol 015 one year in the past. (see fixture client).
    """
...
def clients(sandbox: Sandbox, request) -> Iterator[List[Client]]:
    """N node with protocol 015. Parameterized by the number of nodes.
