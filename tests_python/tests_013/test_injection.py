...
def clients(sandbox):
    """Launches 3 nodes in sandbox mode (genesis, doesn't activate 013)."""
    num_nodes = 3
...
class TestInjectionAndActivation:
    ...
    def test_environment_version(self, clients: List[Client], session: dict):
        proto = session['proto_hash']
        assert clients[0].environment_protocol(proto) == "V6"
...
@pytest.fixture(scope="class")
def client(sandbox):
    """One node in sandbox mode (genesis, doesn't activate 013)."""
    sandbox.add_node(0)
