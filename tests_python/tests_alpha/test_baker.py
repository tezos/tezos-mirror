...
from tools.constants import BOOTSTRAP_BAKERS, IDENTITIES, NODE_PARAMS
...
BAKER_ACTIONS = [
    ...
    (
        'Left { DROP ; NIL baker_operation ;'
        'PUSH signature 0x9ed0297760bb7a4e1037e043f92f59e89b733487c0d96afdbb81'
        'a6dff31fe4bda3ce9957470525c64806bd759a1c8888ce235b01e5b7cb5aa19828d75'
        '20bc20c ;'
        'PUSH key'
        '"edpkv2guuDKmriHf7bVNuPNBQUr4kt2xXnhrN8CCTF7hZ68ruZNdKU" ;'
    ...
    (
        'Left { DROP ; NIL baker_operation ;'
        'PUSH signature 0xdf10efa1133a9922c4b5b936280f1d843043fb84f1c4bdf10776'
        '81954d5d1b0a2cc91a17a2d95a338658a93f4c42646ff5721643e97b1e284d7464d43'
        '8e4600d ;'
        'PUSH key'
...
@pytest.mark.incremental
class TestResetBakerConsensusKey1:
    """Checks that the set_baker_consensus command is effective."""
    ...
    def test_set_consensus_key(self, sandbox):
        client = sandbox.client(0)
        baker_hash = BOOTSTRAP_BAKERS[0]['hash']
        assert (
            client.get_baker_consensus_key(baker_hash, offset=1)
            == IDENTITIES['baker1_key']['public']
        )
        client.gen_key('new_key')
        address = client.show_address('new_key')
        client.set_baker_consensus_key(baker_hash, 'new_key')
        ...
        assert client.get_baker_pending_consensus_key(baker_hash) is None


class TestResetBakerConsensusKey2:
    """Checks that the set_baker_consensus allows resetting the key."""
    ...
    def test_reset_consensus_key(self, sandbox):
        client = sandbox.client(0)
        baker_hash = BOOTSTRAP_BAKERS[1]['hash']

        client.gen_key('new_key_2')
        client.set_baker_consensus_key(baker_hash, 'new_key_2')
        preserved_cycles = protocol.PARAMETERS['preserved_cycles']
        utils.bake_until_nth_cycle_end(
            preserved_cycles, client, 'baker2', BLOCKS_PER_CYCLE, BAKE_ARGS
        )
        client.set_baker_consensus_key(baker_hash, 'baker2_key')
        client.bake('baker2', BAKE_ARGS)
        assert client.get_baker_pending_consensus_key(baker_hash) is None
        utils.bake_many(2, client, 'baker2', BAKE_ARGS)
        assert (
            client.get_baker_consensus_key(baker_hash, offset=1)
            == IDENTITIES['baker2_key']['public']
        )
        assert client.get_baker_pending_consensus_key(baker_hash) is None
