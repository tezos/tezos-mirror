...
class TestMultisig:
    ...
    def test_delegate_change(self, client: Client, session: dict):
        sig0 = client.msig_sign_set_delegate(
            'msig', 'baker5', session['keys'][0]
        )
        to_sign = client.msig_prepare_set_delegate(
            'msig', 'baker5', ['--bytes-only']
        )
        sig2 = client.sign_bytes(to_sign, session['keys'][2])
        client.msig_set_delegate('msig', 'baker5', 'bootstrap1', [sig0, sig2])
        client.bake('baker1', BAKE_ARGS)
...
class TestMultisigFromAddress:
    ...
    def test_delegate_change(self, client: Client, session: dict):
        msig = session['msig']
        sig0 = client.msig_sign_set_delegate(msig, 'baker5', session['keys'][0])
        to_sign = client.msig_prepare_set_delegate(
            msig, 'baker5', ['--bytes-only']
        )
        sig2 = client.sign_bytes(to_sign, session['keys'][2])
