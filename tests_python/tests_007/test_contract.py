...
from .contract_paths import CONTRACT_PATH, ILLTYPED_CONTRACT_PATH, all_contracts
...
class TestManager:
    ...
    def test_transfer_to_manager(self, client: Client):
        ...
        client.transfer(
            amount, 'bootstrap2', 'manager', ['--gas-limit', f'{15450 + 108}']
        )
        client.bake('bootstrap5', BAKE_ARGS)
    ...
    def test_simple_transfer_from_manager_to_implicit(self, client: Client):
        ...
        amount_mutez = utils.mutez_of_tez(amount)
        client.transfer(
            amount, 'manager', 'bootstrap2', ['--gas-limit', f'{26350 + 12}']
        )
        client.bake('bootstrap5', BAKE_ARGS)
        new_balance = client.get_mutez_balance('manager')
        new_balance_bootstrap = client.get_mutez_balance('bootstrap2')
    ...
    def test_simple_transfer_from_manager_to_implicit_json(self, client):
        ...
        amount = 10.1
        amount_mutez = utils.mutez_of_tez(amount)
        client.transfer_json(
            amount, 'manager', 'bootstrap2', ['--default-gas-limit', '26183']
        )
        client.bake('bootstrap5', BAKE_ARGS)
        new_balance = client.get_mutez_balance('manager')
        new_balance_bootstrap = client.get_mutez_balance('bootstrap2')
    ...
    def test_transfer_from_manager_to_manager(self, client: Client):
        ...
        amount_mutez = utils.mutez_of_tez(amount)
        client.transfer(
            amount, 'manager', 'manager2', ['--gas-limit', f'{44950 + 112}']
        )
        client.bake('bootstrap5', BAKE_ARGS)
        new_balance = client.get_mutez_balance('manager')
...
class TestContracts:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("set_update_non_comparable.tz", r'comparable type expected'),
