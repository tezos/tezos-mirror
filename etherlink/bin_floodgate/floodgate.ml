...
let rec get_transaction_receipt rpc_endpoint txn_hash =
  ...
  | None -> get_transaction_receipt rpc_endpoint txn_hash
...
(* Classic deploy bytecode ERC20 contract.
   You can found an example here : https://testnet.explorer.etherlink.com/address/0xB1Ea698633d57705e93b0E40c1077d46CD6A51d8
   You also can fine a list of ERC20 address here : https://docs.etherlink.com/building-on-etherlink/tokens/ *)
...
let run ~scenario ~relay_endpoint ~rpc_endpoint ~controller ~max_active_eoa
  ...
  let* token, gas_limit =
    ...
    | `ERC20 ->
        ...
        in
        let* txn_hash =
          Tx_queue.Misc.send_raw_transaction ~relay_endpoint txn
        in
        let* receipt = get_transaction_receipt rpc_endpoint txn_hash in
        let* contract =
        ...
        in
        Account.increment_nonce controller ;
        let data =
          Efunc_core.Evm.encode
            ~name:"transfer"
            [`address; `uint 256]
