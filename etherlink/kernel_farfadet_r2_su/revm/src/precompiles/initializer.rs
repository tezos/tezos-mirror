...
use crate::{
    ...
    precompiles::constants::{
        FA_BRIDGE_SOL_ADDR, FA_BRIDGE_SOL_CONTRACT, INTERNAL_FORWARDER_SOL_CODE_HASH,
        INTERNAL_FORWARDER_SOL_CONTRACT, WITHDRAWAL_SOL_ADDR, WITHDRAWAL_SOL_CODE_HASH,
...
pub fn init_precompile_bytecodes<Host: Runtime>(host: &'_ mut Host) -> Result<(), Error> {
    ...
    init_precompile_bytecode_predeployed(
        host,
        &FA_BRIDGE_SOL_ADDR,
...
fn init_precompile_bytecode_predeployed<Host: Runtime>(
    host: &'_ mut Host,
    addr: &Address,
