...
// Modified version of https://github.com/axic/snailtracer/blob/master/snailtracer.sol
// Which is a fork of https://github.com/karalabe/snailtracer/blob/master/snailtracer.sol
// Which is an adaptation in EVM of smallpt https://www.kevinbeason.com/smallpt/
...
contract SnailTracer {
  ...
  function Benchmark(uint8 spp) public returns (bytes1 r, bytes1 g, bytes1 b) {
    ...
    color = add(color, trace(512, 384, int(spp))); // Flat diffuse surface, opposite wall
    color = add(color, trace(325, 540, int(spp))); // Reflective surface mirroring left wall
    color = add(color, trace(600, 600, int(spp))); // Refractive surface reflecting right wall
    color = add(color, trace(522, 524, int(spp))); // Reflective surface mirroring the refractive surface reflecting the light
    color = div(color, 4);
