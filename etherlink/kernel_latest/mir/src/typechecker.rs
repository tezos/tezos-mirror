...
impl<'a> Micheline<'a> {
    ...
    pub fn split_script(
        &self,
    ) -> Result<MichelineContractScript<&'a Micheline<'a>>, TcError> {
        let seq = match self {
    ...
    if with_entrypoints {
        if let Option::Some(eps) = entrypoints {
            ...
            let Micheline::App(_prim, _args, anns) = ty else {
                return Err(TcError::InternalError(TcInvariant::ExpectedMichelineApp));
            };
        ...
        match frame {
            ...
            PtyFrame::Build {
                ...
                let parsed = match kind {
                    BuildKind::Pair => {
                        let r = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack { expected: "pair r" },
                        ))?;
                        let l = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack { expected: "pair l" },
                        ))?;
                        Type::new_pair(l, r)
                    }
                    BuildKind::Or => {
                        let r = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack { expected: "or r" },
                        ))?;
                        let l = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack { expected: "or l" },
                        ))?;
                        Type::new_or(l, r)
                    }
                    BuildKind::Lambda => {
                        let ret = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack {
                                expected: "lambda ret",
                            },
                        ))?;
                        let arg = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack {
                                expected: "lambda arg",
                            },
                        ))?;
                    ...
                    BuildKind::Map => {
                        ...
                        let v = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack { expected: "map v" },
                        ))?;
                        let k = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack { expected: "map k" },
                        ))?;
                        Type::new_map(k, v)
                    }
                    BuildKind::BigMap => {
                    ...
                    BuildKind::List => {
                        let t = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack { expected: "list t" },
                        ))?;
                        Type::new_list(t)
                    }
                    BuildKind::Set => {
                        let t = results.pop().ok_or(TcError::InternalError(
                            TcInvariant::EmptyResultStack { expected: "set t" },
                        ))?;
            ...
            PtyFrame::EnsureProp { prop } => {
                ...
                let t = results.last().ok_or(TcError::InternalError(
                    TcInvariant::EmptyResultStack {
                        expected: "ensure_prop target",
                    },
                ))?;
    ...
    while let Some(wframe) = frames.pop() {
        ...
        match frame {
            TcIFrame::NextInstr { block, idx } => {
                ...
                match step {
                    ...
                    StepResult::OpenIfCons {
                        ...
                        let saved_nil_stack =
                            std::mem::replace(opt_stack, cons_opt_stack);
                        frames.push(parent);
            ...
            TcIFrame::AfterIfFalse { saved_t_stack } => {
                let f_instructions = block_results.pop().ok_or(
                    TcError::InternalError(TcInvariant::EmptyResultStack {
                        expected: "if false block",
                    }),
                )?;
                let t_instructions = block_results.pop().ok_or(
                    TcError::InternalError(TcInvariant::EmptyResultStack {
                        expected: "if true block",
                    }),
                )?;
            ...
            TcIFrame::AfterIfNoneSome { saved_none_stack } => {
                let some_instructions = block_results.pop().ok_or(
                    TcError::InternalError(TcInvariant::EmptyResultStack {
                        expected: "if_none some",
                    }),
                )?;
                let none_instructions = block_results.pop().ok_or(
                    TcError::InternalError(TcInvariant::EmptyResultStack {
                        expected: "if_none none",
                    }),
                )?;
            ...
            TcIFrame::AfterIfConsNil { saved_cons_stack } => {
                let nil_instructions = block_results.pop().ok_or(
                    TcError::InternalError(TcInvariant::EmptyResultStack {
                        expected: "if_cons nil",
                    }),
                )?;
                let cons_instructions = block_results.pop().ok_or(
                    TcError::InternalError(TcInvariant::EmptyResultStack {
                        expected: "if_cons cons",
                    }),
                )?;
            ...
            TcIFrame::AfterIfLeftRight { saved_left_stack } => {
                let right_instructions = block_results.pop().ok_or(
                    TcError::InternalError(TcInvariant::EmptyResultStack {
                        expected: "if_left right",
                    }),
                )?;
                let left_instructions = block_results.pop().ok_or(
                    TcError::InternalError(TcInvariant::EmptyResultStack {
                        expected: "if_left left",
                    }),
                )?;
            ...
            TcIFrame::AfterLambda {
                ...
                *opt_stack = saved_outer_stack;
                ...
                let parent_results = block_results.last_mut().ok_or(
                    TcError::InternalError(TcInvariant::EmptyResultStack {
                ...
                if for_push {
                    ...
                    parent_results.push(Instruction::Push(RcTypedValue::new(
                        TypedValue::Lambda(Closure::Lambda(lam)),
                    )));
                } else {
            ...
            TcIFrame::AfterPushValue { ty } => {
                ...
                block_results
                    ...
                    .push(Instruction::Push(RcTypedValue::new(value)));
            }
...
fn typecheck_instruction_with_views<'a>(
    ...
    results
        .pop()
        .ok_or(TcError::InternalError(TcInvariant::EmptyResultStack {
            expected: "typecheck_instruction produced one",
        }))
}
    ...
    macro_rules! pop {
        ...
        ($p:path) => {{
            ...
            match v {
                ...
                _ => {
                    return Err(TcError::InternalError(TcInvariant::TypeMismatchOnPop {
    ...
    Ok(StepResult::Done(match (i, stack_slice.as_slice()) {
        ...
        (App(DIP, args, _), ..) => {
            let (opt_height, nested) = match args {
                [Int(height), Seq(nested)] => {
                    (Option::Some(validate_u10(height)?), nested)
                }
        ...
        (App(GT, [], _), [.., t]) => {
            no_overload!(GT, TypesNotEqual(T::Int, (*t).clone()))
        }
        ...
        (App(GE, [], _), [.., t]) => {
            no_overload!(GE, TypesNotEqual(T::Int, (*t).clone()))
        }
        ...
        (App(EQ, [], _), [.., t]) => {
            no_overload!(EQ, TypesNotEqual(T::Int, (*t).clone()))
        }
        ...
        (App(NEQ, [], _), [.., t]) => {
            no_overload!(NEQ, TypesNotEqual(T::Int, (*t).clone()))
        }
        ...
        (App(LE, [], _), [.., t]) => {
            no_overload!(LE, TypesNotEqual(T::Int, (*t).clone()))
        }
        ...
        (App(LT, [], _), [.., t]) => {
            no_overload!(LT, TypesNotEqual(T::Int, (*t).clone()))
        }
        ...
        (App(ABS, [], _), [.., t]) => {
            no_overload!(ABS, TypesNotEqual(T::Int, (*t).clone()))
        }
        ...
        (App(ISNAT, [], _), [.., t]) => {
            no_overload!(ISNAT, TypesNotEqual(T::Int, (*t).clone()))
        }
        ...
        (App(PUSH, [t, v], _), ..) => {
            ...
            match (&t, v) {
                (
                    T::Lambda(tys),
                    raw @ (Seq(instrs) | App(Prim::Lambda_rec, [Seq(instrs)], _)),
                ) => {
        ...
        (App(CAR, [], _), [.., ty]) => {
            no_overload!(CAR, NMOR::ExpectedPair((*ty).clone()))
        }
        ...
        (App(CDR, [], _), [.., ty]) => {
            no_overload!(CDR, NMOR::ExpectedPair((*ty).clone()))
        }
        ...
        (App(UNPAIR, [], _), [.., ty]) => {
            no_overload!(UNPAIR, NMOR::ExpectedPair((*ty).clone()))
        }
        ...
        (App(CONS, [], _), [.., ty, _]) => {
            no_overload!(CONS, NMOR::ExpectedList((*ty).clone()))
        }
        ...
        (App(GET_AND_UPDATE, [], _), [] | [_] | [_, _]) => {
            no_overload!(GET_AND_UPDATE, len 3)
        }
        ...
        (App(TRANSFER_TOKENS, [], _), [] | [_] | [_, _]) => {
            no_overload!(TRANSFER_TOKENS, len 3)
        }
        ...
        (App(SET_DELEGATE, [], _), [.., T::Option(ot)])
            if matches!(ot.as_ref(), T::KeyHash) =>
        {
            pop!();
            stack.push(T::Operation);
            I::SetDelegate
        ...
        (App(CHECK_SIGNATURE, [], _), [] | [_] | [_, _]) => {
            no_overload!(CHECK_SIGNATURE, len 3)
        }
        ...
        (App(LAMBDA | LAMBDA_REC, expect_args!(3 last_seq), _), _) => {
            unexpected_micheline!()
        }
            ...
            if matches!(tickets.as_ref(), (Type::Ticket(_), Type::Ticket(_))) =>
        {
            let Type::Ticket(lt) = &tickets.0 else {
                return Err(TcError::InternalError(TcInvariant::ExpectedTicketLeftJoin));
            };
            let Type::Ticket(rt) = &tickets.1 else {
                return Err(TcError::InternalError(TcInvariant::ExpectedTicketRightJoin));
            };
        ...
        (
            App(CREATE_CONTRACT, [cs], _),
            [.., new_storage, T::Mutez, T::Option(opt_keyhash)],
    ...
    if let Some(prev) = prev {
        ...
        match prev.cmp(next) {
            ...
            std::cmp::Ordering::Greater => {
                Err(TcError::ElementsNotSorted(set_ty.clone()))
            }
        }
    } else {
        Ok(())
...
fn map_key_ordering_check<'a>(
    ...
    if let Some(prev) = prev_key {
        ...
        match prev.cmp(next_key) {
            ...
            std::cmp::Ordering::Greater => {
                Err(TcError::ElementsNotSorted(map_ty.clone()))
            }
        }
    } else {
        Ok(())
...
fn step_typecheck_value<'a, 'b>(
    ...
    match frame {
        ...
        TvFrame::BuildLeft => {
            let v = results.pop().ok_or(TcError::InternalError(
                TcInvariant::EmptyResultStack { expected: "left" },
            ))?;
        ...
        TvFrame::BuildRight => {
            let v = results.pop().ok_or(TcError::InternalError(
                TcInvariant::EmptyResultStack { expected: "right" },
            ))?;
        ...
        TvFrame::BuildSome => {
            let v = results.pop().ok_or(TcError::InternalError(
                TcInvariant::EmptyResultStack { expected: "some" },
            ))?;
        ...
        TvFrame::ListAccum {
            ...
            let elem = results.pop().ok_or(TcError::InternalError(
                TcInvariant::EmptyResultStack {
                    expected: "list elem",
                },
            ))?;
        ...
        TvFrame::MapAccum {
            ...
            MapStage::AwaitingKey { v_expr } => {
                let key = results.pop().ok_or(TcError::InternalError(
                    TcInvariant::EmptyResultStack {
                        expected: "map key",
                    },
                ))?;
            ...
            MapStage::AwaitingValue { key } => {
                let value = results.pop().ok_or(TcError::InternalError(
                    TcInvariant::EmptyResultStack {
                        expected: "map value",
                    },
                ))?;
            ...
            BigMapDiffStage::AwaitingKey { v_expr } => {
                ...
                match v_expr {
                    ...
                    V::App(Prim::None, [], _) => {
                        ...
                        acc.insert(key, None);
                        if let Some((next_elt, rest)) = remaining.split_first() {
                            let (next_k_expr, next_v_expr) =
                                expect_elt(next_elt, &map_ty)?;
                            frames.push(TvFrame::BigMapDiffAccum {
                                remaining: rest,
                                key_t: key_t.clone(),
                                val_t,
        ...
        TvFrame::BuildTicketExplicit { content_type } => {
            let amount = results.pop().ok_or(TcError::InternalError(
                TcInvariant::EmptyResultStack {
                    expected: "ticket amount",
                },
            ))?;
            let content = results.pop().ok_or(TcError::InternalError(
                TcInvariant::EmptyResultStack {
        ...
        TvFrame::BuildTicketLegacy { content_type } => {
            let pair = results.pop().ok_or(TcError::InternalError(
                TcInvariant::EmptyResultStack {
...
fn visit_value<'a, 'b>(
    ...
    match (&t, v) {
        ...
        (
            T::Pair(pt),
            V::App(Prim::Pair, [vl, rest @ ..], _) | V::Seq([vl, rest @ ..]),
        ) if !rest.is_empty() => {
            let (tl, tr) = pt.as_ref();
            frames.push(TvFrame::BuildPair);
            if rest.len() == 1 {
        ...
        (T::Set(ty), V::Seq(vs)) => {
            ...
            if let Some((first, rest)) = vs.split_first() {
                ...
                results.push(TV::Set(RedBlackTreeSet::new()));
            }
        }
        (T::Map(m), V::Seq(vs)) => {
            let (tk, tv) = m.as_ref();
        ...
        (T::ChainId, V::String(str)) => {
            ...
            results.push(TV::ChainId(
                ChainId::from_base58_check(str)
                    .map_err(|x| TcError::ChainIdError(x.into()))?,
            ));
        }
...
fn visit_map_or_bigmap_inmem<'a, 'b>(
    ...
    if let Some((first_elt, rest)) = vs.split_first() {
        ...
        match finalize {
            MapFinalize::Map => results.push(TV::Map(RedBlackTreeMap::new())),
            MapFinalize::BigMapInMem { tk, tv } => {
                results.push(TV::BigMap(BigMap::new(tk, tv, RedBlackTreeMap::new())))
            }
        }
    }
    Ok(())
...
mod typecheck_tests {
    ...
    fn deeply_nested_if_unifies_deep_types() {
        ...
        let mut gas = Gas::new(u32::MAX);
        typecheck_instruction(&if_instr, &mut gas, &mut stack)
            .expect("IF unifies deep types");
    }
    ...
    fn test_push() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::int(1))))
        );
        assert_eq!(stack, expected_stack);
        assert!(gas.milligas().unwrap() < Gas::default().milligas().unwrap());
    }
    ...
    fn test_dip() {
        ...
        assert_eq!(
            typecheck_instruction(
                &parse("DIP 1 {PUSH nat 6}").unwrap(),
                &mut gas,
                &mut stack
            ),
            Ok(Dip(
                Some(1),
                vec![Push(RcTypedValue::new(TypedValue::nat(6)))]
            ))
        );
        assert_eq!(stack, expected_stack);
        assert!(gas.milligas().unwrap() < Gas::default().milligas().unwrap());
    ...
    fn test_loop() {
        ...
        assert_eq!(
            ...
            Ok(Loop(vec![Push(RcTypedValue::new(TypedValue::Bool(true)))]))
        );
        assert_eq!(stack, expected_stack);
        assert!(gas.milligas().unwrap() < Gas::default().milligas().unwrap());
    }
    ...
    fn test_loop_left() {
        ...
        assert_eq!(
            ...
            Ok(LoopLeft(vec![
                Drop(None),
                Push(RcTypedValue::new(TypedValue::new_or(Or::Right(
                    TypedValue::nat(123)
                ))))
            ]))
        );
    ...
    fn test_map_block_fail() {
        ...
        assert_eq!(
            ...
            Ok(Map(
                overloads::Map::List,
                vec![
                    Push(RcTypedValue::new(TypedValue::Bool(true))),
    ...
    fn test_failed_stacks() {
        ...
        test_fail!(
            "{ PUSH (or never unit) (Right Unit); IF_LEFT { NEVER; PUSH int 1 } {} }"
        );
        macro_rules! test_ok {
            ($code:expr) => {
        ...
        test_ok!(
            "{ PUSH bool True; IF { PUSH int 1; FAILWITH } { PUSH int 1; FAILWITH } }"
        );
    ...
    fn push_string_value() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::String(
                "foo".to_owned()
            ))))
        );
    ...
    fn push_bls_fr_int() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::Bls12381Fr(
    ...
    fn push_bls_g1() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::new_bls12381_g1(
    ...
    fn push_bls_g2() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::new_bls12381_g2(
    ...
    fn push_unit_value() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::Unit)))
        );
    ...
    fn push_pair_value() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::new_pair(
                TypedValue::int(-5),
    ...
    fn push_or_value_left() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::new_or(or::Or::Left(
                TypedValue::int(1)
            )))))
        );
    ...
    fn push_or_value_right() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::new_or(or::Or::Right(
    ...
    fn push_option_value() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::new_option(Some(
                TypedValue::nat(3)
            )))))
        );
    ...
    fn push_option_none_value() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::new_option(None))))
        );
        assert_eq!(stack, tc_stk![Type::new_option(Type::Nat)]);
    }
    ...
    fn car() {
        ...
        assert_eq!(
            ...
            Ok(Seq(vec![
                Push(RcTypedValue::new(TypedValue::new_pair(
                    TypedValue::int(-5),
                    TypedValue::new_pair(TypedValue::nat(3), TypedValue::Bool(false))
                ))),
                Car
    ...
    fn if_none() {
        ...
        assert_eq!(
            ...
            Ok(IfNone(
                vec![Push(RcTypedValue::new(TypedValue::int(5)))],
                vec![]
            ))
        );
        assert_eq!(stack, tc_stk![Type::Int]);
    ...
    fn push_int_list() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::List(
    ...
    fn push_set() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::Set(
    ...
    fn push_map() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::Map(
                [
    ...
    fn push_map_incomparable() {
        ...
        assert_eq!(
            typecheck_instruction(
                &parse(
                    r#"PUSH (map (list int) string) { Elt { 2 } "foo"; Elt { 1 } "bar" }"#
                )
                .unwrap(),
                &mut Gas::default(),
                &mut stack
    ...
    fn push_map_wrong_key_type() {
        ...
        assert_eq!(
            typecheck_instruction(
                &parse(r#"PUSH (map int string) { Elt "1" "foo"; Elt 2 "bar" }"#)
                    .unwrap(),
                &mut Gas::default(),
                &mut stack
            ),
            ...
            ($op:ident, $instr:expr) => {
                ...
                mod $op {
                    ...
                    fn ok() {
                        ...
                        assert_eq!(stack, expected_stack);
                        assert!(
                            gas.milligas().unwrap() < Gas::default().milligas().unwrap()
                        );
                    }
                    ...
                    fn mismatch() {
                        ...
                        assert_eq!(
                            ...
                            Err(TcError::NoMatchingOverload {
                                ...
                                stack: stk![Type::String],
                                reason: Some(
                                    TypesNotEqual(Type::Int, Type::String).into()
                                )
                            })
                        );
                    }
        ...
        assert_eq!(
            ...
            Ok(ContractScript {
                ...
                code: Seq(vec![
                    ...
                    Nil,
                    Pair,
                    Push(RcTypedValue::new(TypedValue::Address(addr::Address
    ...
    fn test_push_address() {
        ...
        fn test_ok(lit: &str, bytes: &str, exp: addr::Address) {
            let exp = Ok(Push(RcTypedValue::new(TypedValue::Address(exp))));
            assert_eq!(
                &typecheck_instruction(
        ...
        assert_matches!(
            typecheck_instruction(
                &parse("PUSH address 0x00fffe0000000000000000000000000000000000000000")
                    .unwrap(),
                &mut Gas::default(),
                &mut tc_stk![],
            ),
        ...
        assert_matches!(
            typecheck_instruction(
                &parse("PUSH address 0x03d601f22256d2ad1faec0c64374e527c6e62f2e5a666f6f")
                    .unwrap(),
                &mut Gas::default(),
                &mut tc_stk![],
            ),
    ...
    fn test_push_address_l2_1377_relaxed_entrypoint() {
        ...
        fn assert_ok(lit: &str, bytes: &str, ep: &str) {
            ...
            let exp = Ok(Push(RcTypedValue::new(TypedValue::Address(exp_addr))));
            assert_eq!(
                &typecheck_instruction(
                    &parse(&format!("PUSH address \"{lit}\"")).unwrap(),
                    &mut Gas::default(),
        ...
        // literal is always valid UTF-8. L1 accepts it; MIR must too.
        {
            ...
            assert_eq!(
                &typecheck_instruction(
                    &parse(
                        "PUSH address 0x00007b09f782e0bcd67739510afa819d85976119d5efff"
                    )
                    .unwrap(),
                    &mut Gas::default(),
                    &mut tc_stk![],
        ...
        assert_matches!(
            typecheck_instruction(
                &parse("PUSH address \"tz1WrbkDrzKVqcGXkjw4Qk4fXkjXpAJuNP1j%default\"")
                    .unwrap(),
                &mut Gas::default(),
                &mut tc_stk![],
            ),
    ...
    fn test_push_chain_id() {
        ...
        let exp = Ok(Push(RcTypedValue::new(TypedValue::ChainId(
    ...
    fn self_instr_contract_overlong_field_ann() {
        ...
        assert_eq!(
            ...
            Ok(ContractScript {
                ...
                annotations: HashMap::from([
                    (
                        FieldAnnotation::from_str_unchecked(
    ...
    fn dip_dup_arg_too_large() {
        assert_eq!(
            parse("DROP 1025").unwrap().typecheck_instruction(
                &mut Gas::default(),
                None,
                &[]
            ),
        ...
        assert_eq!(
            parse("DIP 1024 {}").unwrap().typecheck_instruction(
                &mut Gas::default(),
                None,
                &[]
            ),
        ...
        assert_eq!(
            parse("DUP 65536").unwrap().typecheck_instruction(
                &mut Gas::default(),
                None,
                &[]
            ),
    ...
    fn push_bytes() {
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::Bytes(
    ...
    fn push_key() {
        assert_eq!(
            parse("PUSH key \"p2pk67K1dwkDFPB63RZU5H3SoMCvmJdKZDZszc7U4FiGKN2YypKdDCB\"")
                .unwrap()
                .typecheck_instruction(&mut Gas::default(), None, &[]),
            Ok(Push(RcTypedValue::new(TypedValue::Key(
    ...
    fn push_key_hash() {
        assert_eq!(
            parse("PUSH key_hash \"tz1Nw5nr152qddEjKT2dKBH8XcBMDAg72iLw\"")
                .unwrap()
                .typecheck_instruction(&mut Gas::default(), None, &[]),
            Ok(Push(RcTypedValue::new(TypedValue::KeyHash(
    ...
    fn push_signature() {
        assert_eq!(
            ...
            .unwrap()
            .typecheck_instruction(&mut Gas::default(), None, &[]),
            Ok(Push(RcTypedValue::new(TypedValue::Signature(
    ...
    fn push_lambda() {
        assert_eq!(
            parse("PUSH (lambda unit unit) { DROP ; UNIT }")
                .unwrap()
                .typecheck_instruction(&mut Gas::default(), None, &[]),
            Ok(Push(RcTypedValue::new(TypedValue::Lambda(
    ...
    fn push_lambda_rec_bad_result() {
        assert_eq!(
            ...
            Err(TcError::StacksNotEqual(
                ...
                stk![Type::Unit],
                TypesNotEqual(Type::new_lambda(Type::Unit, Type::Unit), Type::Unit)
                    .into()
            ))
        );
        assert_eq!(
    ...
    fn typecheck_forged_ticket() {
        ...
        let ticket: super::Ticket = super::Ticket {
            ticketer: AddressHash::try_from("tz1T1K14rZ46m1GT1kPVwZWkSHxNSDZgM71h")
                .unwrap(),
    ...
    fn lambda_typecheck_depth_counter_balances_on_push_lambda_body_err() {
        ...
        // path.
        let err =
            crate::parser::test_helpers::parse("PUSH (lambda unit int) { DROP ; UNIT }")
                .unwrap()
                .typecheck_instruction(&mut Gas::default(), None, &[])
                .unwrap_err();
        assert!(
    ...
    // `TypedValue -> Closure::Lambda -> Rc<[Instruction]> -> Push(RcTypedValue)
...
code { DROP ;
  ...
  NIL operation ; PAIR }"#)
    ...
    fn map_key_comparable_check_fires_before_value_parsing() {
        ...
        let err = parse_ty(
            &mut Gas::default(),
            &parse("map (list unit) (pair int)").unwrap(),
        )
                    ...
                    fn bad_input() {
                        ...
                        assert_eq!(
                            ...
                            Err(TcError::NoMatchingOverload {
                                ...
                                stack: stk![Type::Unit],
                                reason: Some(
                                    TypesNotEqual(Type::Bytes, Type::Unit).into()
                                ),
                            }),
                        );
                    }
                    ...
                    fn bad_micheline() {
                        assert!(matches!(
                            typecheck_instruction(
                                ...
                                &parse(&format!("{} unit", stringify!($instr_prim)))
                                    .unwrap(),
                                &mut Gas::default(),
                                &mut tc_stk![],
                            ),
    ...
    fn timestamp_value_tc() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::timestamp(1571659294))))
        );
    ...
    fn timestamp_value_string_bigint() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::Timestamp(
                expected.clone()
            ))))
        );
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::Timestamp(expected))))
        );
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::Timestamp(neg_expected))))
        );
    }
    ...
    fn timestamp_value_string_leap_second() {
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::timestamp(60))))
        );
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::timestamp(0))))
        );
        ...
        assert_eq!(
            ...
            Ok(Push(RcTypedValue::new(TypedValue::timestamp(59))))
        );
    }
    ...
    fn emit() {
        ...
        assert_eq!(
            ...
            Err(TcError::UnexpectedMicheline(
                "App(EMIT, [App(nat, [], []), App(nat, [], [])], [Field(\"mytag\")])"
                    .to_string()
            ))
        );
    ...
    fn set_delegate_in_if_in_view_forbidden() {
        assert_eq!(
            typecheck_script_with_view(
                "DROP; PUSH bool True; IF { SET_DELEGATE } { UNIT }"
            ),
            Err(TcError::ForbiddenInView(Prim::SET_DELEGATE))
        );
    }
    ...
    fn stack_get_oob_internal_error_format() {
        ...
        let err =
            super::stack_get(&stack, 3).expect_err("out-of-range access must error");
        assert_eq!(
            format!("{err}"),
