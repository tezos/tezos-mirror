...
pub(crate) const TEZOSX_RUN_CODE_INPUT: RefPath =
    RefPath::assert_from(b"/base/tezosx_run_code/input");
pub(crate) const TEZOSX_RUN_CODE_RESULT: RefPath =
    RefPath::assert_from(b"/base/tezosx_run_code/result");
...
{
    let mut host: KernelHost<Host, &mut Host> = KernelHost::init(host);
    let result = match host.store_read_all(&TEZOSX_RUN_CODE_INPUT) {
        ...
        Err(err) => {
            RunCodeResult::HostError(format!("run_code input not found: {err:?}"))
        }
        Ok(payload) => run_code_from_input(&mut host, &payload).into(),
    };
    ...
    if let Err(err) = host.store_write_all(&TEZOSX_RUN_CODE_RESULT, &bytes) {
        log!(
            Error,
    ...
    Host: std::borrow::BorrowMut<R> + std::borrow::Borrow<R>,
{
    ...
    let blueprint_header = read_current_blueprint_header(host).map_err(|err| {
    ...
    let mut safe_host = SafeStorage {
        ...
        world_states: chain_config
            .storage_root_paths(blueprint_header.number)
            .into_iter()
            .map(OwnedPath::from)
            .collect(),
    };
    ...
    let result =
        tezos_execution::run_code(&mut safe_host, &registry, &mut journal, &params);
