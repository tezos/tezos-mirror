...
use crate::account_storage::{
    read_u256, write_u256, AccountStorageError, EthereumAccount,
};
...
impl WithdrawalCounter for EthereumAccount {
    fn withdrawal_counter_get_and_increment(
        ...
        let old_value = read_u256(host, &path, U256::zero())?;
        let new_value = old_value
            .checked_add(U256::one())
            .ok_or(AccountStorageError::NonceOverflow)?;

        write_u256(host, &path, new_value)?;
        Ok(old_value)
    }
}
...
mod tests {
    ...
    use crate::{
        account_storage::{read_u256, EthereumAccount},
    ...
    fn withdrawal_counter_initializes_and_increments() {
        ...
        let next_id =
            read_u256(&mock_host, &RefPath::assert_from(path), U256::zero()).unwrap();
