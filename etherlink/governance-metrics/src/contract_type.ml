...
let decode_finish_vote finished_vote =
  let open Result_syntax in
  let decode = function
    | [period_index; period_type; winner_proposal] ->
        ...
        Ok {finished_at = period; winner_proposal_payload}
    | _ -> error_with "DecodeFinishVoteError"
  in
  decode_pair decode finished_vote
...
let decode_voting_state voting_state =
  ...
  let decode = function
    | [period_index; period_type; remaining_blocks; finish_vote] ->
        ...
        Ok {current_period; remaining_blocks; finished_voting}
