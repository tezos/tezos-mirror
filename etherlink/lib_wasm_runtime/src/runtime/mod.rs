...
impl NativeKernel {
    fn runtime_version(&self) -> RuntimeVersion {
        match self {
            ...
            Self::FarfadetR6 => RuntimeVersion::V1,
        }
    }
}
...
impl NativeKernel {
    fn of_root_hash(root_hash: &ContextHash) -> Option<NativeKernel> {
        ...
        match root_hash_hex.as_str() {
            ...
            FARFADET_R6_ROOT_HASH_HEX => Some(NativeKernel::FarfadetR6),
            _ => None,
        }
    }
}
    ...
    fn call(&mut self) -> Result<(), Error> {
        match (self.entrypoint.as_str(), self.variant) {
            ...
            ("kernel_run", NativeKernel::FarfadetR2) => {
                trace!("farfadet_r2_su::kernel_loop");
                let hasher = self.host().hasher();
                kernel_farfadet_r2_su::kernel(self.mut_host(), hasher);
                Ok(())
            }
            ("populate_delayed_inbox", NativeKernel::FarfadetR2) => {
                trace!("farfadet_r2_su::populate_delayed_inbox");
                let hasher = self.host().hasher();
                kernel_farfadet_r2_su::evm_node_entrypoint::populate_delayed_inbox_with_durable_storage(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            ...
            ("single_tx_execution", NativeKernel::FarfadetR2) => {
                trace!("farfadet_r2_su::single_tx_execution");
                let hasher = self.host().hasher();
                kernel_farfadet_r2_su::evm_node_entrypoint::single_tx_execution_fn(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            ...
            ("assemble_block", NativeKernel::FarfadetR2) => {
                trace!("farfadet_r2_su::assemble_block");
                let hasher = self.host().hasher();
                kernel_farfadet_r2_su::evm_node_entrypoint::assemble_block_fn(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            }
            ("kernel_run", NativeKernel::FarfadetR3) => {
                trace!("farfadet_r3_su::kernel_loop");
                let hasher = self.host().hasher();
                kernel_farfadet_r3_su::kernel(self.mut_host(), hasher);
                Ok(())
            }
            ("populate_delayed_inbox", NativeKernel::FarfadetR3) => {
                trace!("farfadet_r3_su::populate_delayed_inbox");
                let hasher = self.host().hasher();
                kernel_farfadet_r3_su::evm_node_entrypoint::populate_delayed_inbox_with_durable_storage(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            }
            ("single_tx_execution", NativeKernel::FarfadetR3) => {
                trace!("farfadet_r3_su::single_tx_execution");
                let hasher = self.host().hasher();
                kernel_farfadet_r3_su::evm_node_entrypoint::single_tx_execution_fn(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            }
            ("assemble_block", NativeKernel::FarfadetR3) => {
                trace!("farfadet_r3_su::assemble_block");
                let hasher = self.host().hasher();
                kernel_farfadet_r3_su::evm_node_entrypoint::assemble_block_fn(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            }
            ("kernel_run", NativeKernel::FarfadetR4) => {
                trace!("farfadet_r4_su::kernel_loop");
                let hasher = self.host().hasher();
                kernel_farfadet_r4_su::kernel(self.mut_host(), hasher);
                Ok(())
            }
            ("populate_delayed_inbox", NativeKernel::FarfadetR4) => {
                trace!("farfadet_r4_su::populate_delayed_inbox");
                let hasher = self.host().hasher();
                kernel_farfadet_r4_su::evm_node_entrypoint::populate_delayed_inbox_with_durable_storage(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            }
            ("single_tx_execution", NativeKernel::FarfadetR4) => {
                trace!("farfadet_r4_su::single_tx_execution");
                let hasher = self.host().hasher();
                kernel_farfadet_r4_su::evm_node_entrypoint::single_tx_execution_fn(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            }
            ("assemble_block", NativeKernel::FarfadetR4) => {
            ...
            ("kernel_run", NativeKernel::FarfadetR6) => {
                trace!("farfadet_r6_su::kernel_loop");
                let hasher = self.host().hasher();
                kernel_farfadet_r6_su::kernel(self.mut_host(), hasher);
                Ok(())
            }
            ("populate_delayed_inbox", NativeKernel::FarfadetR6) => {
                trace!("farfadet_r6_su::populate_delayed_inbox");
                let hasher = self.host().hasher();
                kernel_farfadet_r6_su::evm_node_entrypoint::populate_delayed_inbox_with_durable_storage(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            }
            ("single_tx_execution", NativeKernel::FarfadetR6) => {
                trace!("farfadet_r6_su::single_tx_execution");
                let hasher = self.host().hasher();
                kernel_farfadet_r6_su::evm_node_entrypoint::single_tx_execution_fn(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
            }
            ("assemble_block", NativeKernel::FarfadetR6) => {
                trace!("farfadet_r6_su::assemble_block");
                let hasher = self.host().hasher();
                kernel_farfadet_r6_su::evm_node_entrypoint::assemble_block_fn(
                    self.mut_host(),
                    hasher,
                );
                Ok(())
