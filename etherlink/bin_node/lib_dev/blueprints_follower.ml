...
let local_head_too_old ?remote_head ~chain_family ~evm_node_endpoint
    ...
    | None | Some _ -> (
        ...
        | L2_types.EVM ->
            Batch.call
              ...
              ()
        | L2_types.Michelson ->
            (* Node and kernel is probably in Tezlink mode *)
            Batch.call
...
module Blueprints_sequence = struct
  ...
  let make_legacy_rpc ~chain_family ~next_blueprint_number evm_node_endpoint : t
      =
    let open Lwt_result_syntax in
    Seq_es.ES.unfold
  ...
  let rec make_with_chunks ?remote_head ~chain_family ~next_blueprint_number
      evm_node_endpoint : t =
   fun () ->
    let open Lwt_result_syntax in
    let* is_too_old, remote_head =
  ...
  let make ~chain_family ~next_blueprint_number evm_node_endpoint : t =
    if !legacy_rpc_fallback then
      make_legacy_rpc ~chain_family ~next_blueprint_number evm_node_endpoint
    else make_with_chunks ~chain_family ~next_blueprint_number evm_node_endpoint
end

let rec catchup ~chain_family ~next_blueprint_number ~first_connection params :
...
and stream_loop ~chain_family (Qty next_blueprint_number) params stream =
  let open Lwt_result_syntax in
...
let start ?(ping_tx_pool = true) ~chain_family ~time_between_blocks
