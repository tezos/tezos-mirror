...
let gas_constants ~storage_version =
  let capacity, alpha =
    if storage_version < 35 (* Calypso or before *) then
      (4_000_000, 0.000_000_007)
    else if storage_version < 38 then (* Dionysus *) (8_000_000, 0.000_000_004)
    else if storage_version < 44 then (* Ebisu *) (14_000_000, 0.000_000_0019)
...
let constants_from_storage_version storage_version =
  (* See migrations V32 in [migration.rs] *)
  if storage_version < 32 then ticks_constants
  else gas_constants ~storage_version
