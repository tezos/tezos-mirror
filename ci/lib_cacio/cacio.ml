...
module Make (Component : COMPONENT) : COMPONENT_API = struct
          ...
          List.flatten
            [
              [
                ...
                ( "SCCACHE_GCS_RW_MODE",
                  match policy with
                  | Some Pull | None -> "READ_ONLY"
                  | Some (Pull_push | Push) -> "READ_WRITE" );
              ...
              | None -> []
              | Some x -> [("SCCACHE_ERROR_LOG", x)]);
              (match log with None -> [] | Some x -> [("SCCACHE_LOG", x)]);
              variables;
            ]
    in
    let before_script =
      match sccache with
      | None -> before_script
      | Some _ -> ". ./scripts/ci/sccache-start.sh" :: before_script
    in
    let after_script =
      match sccache with
      | None -> after_script
      | Some _ -> "./scripts/ci/sccache-stop.sh" :: after_script
    in
    {
      uid = fresh_uid ();
      source_location;
  ...
  let tezt_job ~__POS__:source_location ~pipeline ~description ?provider ?arch
    ...
    let variables_to_echo =
      ...
      ["TESTS"; "CI_NODE_INDEX"; "CI_NODE_TOTAL"; "TEZT_PARALLEL"]
    in
