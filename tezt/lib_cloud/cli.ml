...
type config = {
  ...
  retrieve_daily_logs : string option;
...
let daily_logs_typ : string option Clap.typ =
  Clap.typ
    ~name:"daily_logs"
    ~dummy:None
    ~parse:(fun s ->
      if Sys.file_exists s then (
        Log.error
          "The destination folder of --retrieve-daily-logs already exists: %s"
          s ;
        None)
      else if proxy then (
        Log.warn
          "The --retrieve-daily-logs option is not available when --proxy is \
           used." ;
        Some None)
      else Some (Some s))
...
let retrieve_daily_logs =
  let from_cli =
    Clap.default
      ~section
      ~long:"retrieve-daily-logs"
      ~description:
        "Retrieves the daily logs, usually info logs, that are generated by \
         the daemons, and stores it at the given path. This can represent \
         quite a huge quantity of data. Set to [false] by default."
      daily_logs_typ
      None
  in
  Option.fold ~none:config.retrieve_daily_logs ~some from_cli
