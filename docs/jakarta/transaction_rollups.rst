...
.. note::
   ...
   can use the ``013-PtJakart`` binaries on testnets like `Jakartanet
   <https://jakartanet.tzkt.io/>_` (``--network jakartanet``). Note
   that, compared to testnets like Mondaynet or Dailynet, Jakartanet
   is using the same protocol constants as Mainnet, meaning the
   finality period is 40,000 blocks (about 2 weeks).
...
.. code:: sh
    ...
    tezos-tx-rollup-node-013-PtJakart init ${mode} config \
...
.. code:: sh
    ...
    tezos-tx-rollup-client-013-PtJakart get balance for alice of ${ticket_hash}
...
.. code:: sh
    ...
    tezos-tx-rollup-client-013-PtJakart transfer ${qty} \
          of ${ticket_hash} \
          from ${src} to ${dst} \
...
.. code:: sh
    ...
    tezos-tx-rollup-client-013-PtJakart get block ${block_id}
...
.. code:: sh
    ...
    tezos-tx-rollup-client-013-PtJakart withdraw ${qty} of ${ticket_hash} from ${l2_src} to ${l1_dst}
...
.. code:: sh
    ...
    tezos-tx-rollup-client-013-PtJakart rpc get "/context/head/tickets/${ticket_hash}"
