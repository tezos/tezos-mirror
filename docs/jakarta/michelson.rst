...
the :ref:`concrete syntax <ConcreteSyntax_jakarta>`. In particular
...
-  ``big_map (k) (t)``: Lazily deserialized maps from keys of type
   ...
   See the section on :ref:`operations on big maps <OperationsOnBigMaps_jakarta>` for a description of the syntax of values of type ``big_map (k) (t)`` and available operations.
...
:ref:`special annotations <SpecialAnnotations_jakarta>` and are not allowed everywhere.
...
The multisig commands of :ref:`Tezos command line client <client_manual_jakarta>`
use this
