...
Note that :ref:`manager operations batches <manager_operations_batches_u025>` contain a sequence of external operations signed as a whole by a source user account, which are executed atomically.
...
the :ref:`concrete syntax <ConcreteSyntax_u025>`. In particular
...
However, one side effect of this RPC is the stripping of :ref:`annotations <annotations_u025>`.
...
Note that all annotations are stripped during :ref:`type normalization <type_normalization_u025>`.
...
:ref:`special annotations <SpecialAnnotations_u025>` and are not allowed everywhere.
...
The multisig commands of :ref:`Octez command line client <client_manual_u025>`
use this
...
:ref:`similar to Michelson scripts <syntax_of_scripts_u025>` but
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_u025>`).
...
:ref:`syntax of errors <syntax_of_errors_u025>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_u025>` can
...
- ``other_contracts`` (optional, defaults to ``{}``): mapping between
  ...
  specifications <syntax_of_other_contracts_u025>`)
...
<syntax_of_concrete_operations_u025>`) or implementation-specific
...
:ref:`wildcard pattern <omitting_parts_of_the_output_u025>` in the
...
- ``(StaticError <error description>)``: an error occurred before the
  ...
  <omitting_parts_of_the_output_u025>` such as ``(StaticError _)``
...
<omitting_parts_of_the_output_u025>`.
