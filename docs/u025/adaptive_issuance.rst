...
At the end of each blockchain :ref:`cycle <def_cycle_u025>`, the
...
adaptive issuance rate is the sum of a :ref:`static rate <static_rate_u025>`
and a :ref:`dynamic rate <dynamic_rate_u025>`. This value is kept within
...
- ``issuance_modification_delay`` is a :ref:`derived protocol
  constant<protocol_constants_u025>` which is set to the same value
  as ``CONSENSUS_RIGHTS_DELAY`` (see :ref:`active_stake_u025`), that
  is, 2 cycles.
...
.. note::
  ...
  rate<minimum_and_maximum_rates_u025>` is reached, it is possible,
...
<minimum_and_maximum_rates_u025>`, along with the :ref:`adaptive
maximum <adaptive_maximum_u025>`, computed for the cycle ``c + 1``.
...
<rewards_weights_u025>`.

.. _rewards_weights_u025:
...
for contributing to :ref:`random seed generation <randomness_generation_u025>`
