...
:ref:`slots<slots_u025>`, called **baking rights**, to each
:ref:`delegate a.k.a. baker<def_delegate_u025>`. For this selection
process, each baker is weighted according to its **baking power** --
provided that it is :ref:`active<active_delegate_u025>` and meets the
:ref:`minimal power and own staked
requirements<minimal_baking_power_u025>`.
...
each delegate's :ref:`voting power<voting_power_u025>` instead, which
is computed in a similar but simpler way.


.. _RPC_path_shortcut_u025:
...
.. _baking_power_overview_u025:

Overview
--------

At the end of :ref:`cycle<def_cycle_u025>` ``n`` (that is, the
...
:ref:`requirements<minimal_baking_power_u025>`. (``CONSENSUS_RIGHTS_DELAY
= 2`` is a :ref:`protocol constant<cs_constants_u025>`.)
...
account<user_accounts_u025>` that has registered as a delegate by
emitting a self-``delegation`` :ref:`manager
operation<manager_operations_u025>`. The list of all registered
...
emitting a :ref:`stake operation<staked_funds_management_u025>`. This
...
be :ref:`slashed<slashing_u025>` if the delegate misbehaves. That's
...
- The **unstaked balance** is the sum of the tez amounts contained in
  ...
  :ref:`staked_funds_management_u025`. Unstake requests can be
...
- The **frozen bonds** are a deposit for :ref:`rollup
  commitments<commitments_u025>`. They can be queried with RPC
...
least four cycles<staked_funds_management_u025>`, so short-duration
...
parameter<staking_policy_configuration_u025>` that limits how much
...
:ref:`min_delegated_in_current_cycle<min_delegated_u025>`, and any
:ref:`overstaked<overstaking_u025>` tez count as delegated
too. Therefore:
...
:ref:`as defined above<baking_power_overview_u025>`:
...
- The delegate must be :ref:`active<active_delegate_u025>`

where ``MINIMAL_STAKE = 6,000ꜩ`` and ``MINIMAL_FROZEN_STAKE = 600ꜩ``
are :ref:`protocol constants<cs_constants_u025>`.
