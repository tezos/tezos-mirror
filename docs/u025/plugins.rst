...
:src:`src/proto_025_PsGXQmSv/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_u025`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_u025`).

.. _prevalidator_filters_u025:
...
:ref:`filter RPCs<active_filter_rpc_u025>`) parameters of the filter
configuration of your node.

.. _consensus_filter_u025:
...
RPCs<active_filter_rpc_u025>`) parameter of the filter configuration of your
node.
...
pass<validation_passes_u025>`. Some extra information (like the fees, or the
