...
-  **Stability**: Binaries from a trusted repository undergo rigorous
   ...
   We make sure to compile our binaries in a clean environenment and
...
-  **Ease of Installation**: Binary packages can be installed using
   ...
   ubiquitous in the Debian world. They allow us to sign our packages and
   the end user to automatically verify their installation. We
...
These binaries can be used with any user. However, to run Octez in
...
In particular, to accelerate the node's bootstrap, you usually :ref:`import a snapshot file <importing_a_snapshot>` before starting the node.
...
are stored by default in ``/var/log/tezoz/node.log``. Logs are
...
function, we suggest hereafter a sligtly more involved configuration procedure
...
The logs of the baker are available in ``/var/logs/tezos/baker-active.log``.
...
name of the protocols associated with these daemons are specified in
...
possible and recomended to avoid downtime.
