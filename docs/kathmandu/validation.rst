...
modes<validation_modes_kathmandu>`. For example, these modes enable the
...
.. _protocol_classification_kathmandu:
...
constitute a sufficiently large :ref:`quorum<quorum_kathmandu>`. That is,
...
block (see :ref:`partial_application_kathmandu` above). This feature
...
Unfortunately, the implementation of these two morally distinct phases
in previous Tezos economic protocols has been entangled within the
``apply_operation`` method. As a result, using the latter method did
not allow in practice to distinguish between invalid and
valid-yet-inapplicable operations, and external *ad hoc* mechanisms
were developed to circumvent this limitation -- see the :ref:`precheck
prevalidator filter<precheck_filter_kathmandu>` entry.

This economic protocol implements the first stage of a pipelined
validation discipline which targets to reflect, operationally, this
separation of concerns. However, in this economic protocol, only
manager operations are allowed to fail during the application phase
and yet still be included in a block.

The explicit conditions for a manager operation to be considered as
valid, are given in the :ref:`manager_operations_validity_alpha`
sub-section below.
...
:ref:`Manager operation<manager_operations_kathmandu>` are a class of
...
batch, as defined above. However, it also entails some additional
*global* constraints on manager batches.
