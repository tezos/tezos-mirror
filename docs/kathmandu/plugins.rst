...
:src:`src/proto_014_PtKathma/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_kathmandu`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_kathmandu`).

.. _prevalidator_filters_kathmandu:
