...
At the end of each blockchain :ref:`cycle <def_cycle_r022>`, the
...
adaptive issuance rate is the sum of a :ref:`static rate <static_rate_r022>`
and a :ref:`dynamic rate <dynamic_rate_r022>`. This value is kept within
...
- ``issuance_modification_delay`` is a :ref:`derived protocol
  constant<protocol_constants_r022>` which is set to the same value
  as ``CONSENSUS_RIGHTS_DELAY`` (see :ref:`active_stake_r022`), that
  is, 2 cycles.
...
.. note::
  ...
  rate<minimum_and_maximum_rates_r022>` is reached, it is possible,
...
<minimum_and_maximum_rates_r022>`, along with the :ref:`adaptive
maximum <adaptive_maximum_r022>`, computed for the cycle ``c + 1``.
...
<rewards_weights_r022>`.

.. _rewards_weights_r022:
...
for contributing to :ref:`random seed generation <randomness_generation_r022>`
