...
Note that :ref:`manager operations batches <manager_operations_batches_r022>` contain a sequence of external operations signed as a whole by a source user account, which are executed atomically.
...
the :ref:`concrete syntax <ConcreteSyntax_r022>`. In particular
...
However, one side effect of this RPC is the stripping of :ref:`annotations <annotations_r022>`.
...
Note that all annotations are stripped during :ref:`type normalization <type_normalization_r022>`.
...
:ref:`special annotations <SpecialAnnotations_r022>` and are not allowed everywhere.
...
The multisig commands of :ref:`Octez command line client <client_manual_r022>`
use this
...
:ref:`similar to Michelson scripts <syntax_of_scripts_r022>` but
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_r022>`).
...
:ref:`syntax of errors <syntax_of_errors_r022>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_r022>` can
...
- ``other_contracts`` (optional, defaults to ``{}``): mapping between
  ...
  specifications <syntax_of_other_contracts_r022>`)
...
.. _syntax_of_concrete_stacks_r022:
...
.. _omitting_parts_of_the_output_r022:
...
<syntax_of_concrete_operations_r022>`) or implementation-specific
parts of error outputs (see the :ref:`syntax of errors
<syntax_of_errors_r022>`).

.. _output_normalization_r022:
...
:ref:`wildcard pattern <omitting_parts_of_the_output_r022>` in the
...
- ``(StaticError <error description>)``: an error occurred before the
  ...
  <omitting_parts_of_the_output_r022>` such as ``(StaticError _)``
...
.. _syntax_of_concrete_operations_r022:
...
<omitting_parts_of_the_output_r022>`.
...
.. _syntax_of_other_contracts_r022:
...
.. _syntax_of_extra_big_maps_r022:
