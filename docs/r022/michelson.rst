...
Note that :ref:`manager operations batches <manager_operations_batches_next>` contain a sequence of external operations signed as a whole by a source user account, which are executed atomically.
...
the :ref:`concrete syntax <ConcreteSyntax_next>`. In particular
...
However, one side effect of this RPC is the stripping of :ref:`annotations <annotations_next>`.
...
Note that all annotations are stripped during :ref:`type normalization <type_normalization_next>`.
...
:ref:`special annotations <SpecialAnnotations_next>` and are not allowed everywhere.
...
The multisig commands of :ref:`Octez command line client <client_manual_next>`
use this
...
:ref:`similar to Michelson scripts <syntax_of_scripts_next>` but
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_next>`).
...
:ref:`syntax of errors <syntax_of_errors_next>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_next>` can
...
- ``other_contracts`` (optional, defaults to ``{}``): mapping between
  ...
  specifications <syntax_of_other_contracts_next>`)
...
.. _syntax_of_concrete_stacks_next:
...
.. _omitting_parts_of_the_output_next:
...
<syntax_of_concrete_operations_next>`) or implementation-specific
parts of error outputs (see the :ref:`syntax of errors
<syntax_of_errors_next>`).

.. _output_normalization_next:
...
:ref:`wildcard pattern <omitting_parts_of_the_output_next>` in the
...
- ``(StaticError <error description>)``: an error occurred before the
  ...
  <omitting_parts_of_the_output_next>` such as ``(StaticError _)``
...
.. _syntax_of_concrete_operations_next:
...
<omitting_parts_of_the_output_next>`.
...
.. _syntax_of_other_contracts_next:
...
.. _syntax_of_extra_big_maps_next:
