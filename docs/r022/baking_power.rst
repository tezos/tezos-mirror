...
:ref:`slots<slots_next>`, called **baking rights**, to each
:ref:`delegate a.k.a. baker<def_delegate_next>`. For this selection
process, each baker is weighted according to its **baking power** --
provided that it is :ref:`active<active_delegate_next>` and meets the
...
each delegate's :ref:`voting power<voting_power_next>` instead, which
is computed in a similar but simpler way.


.. _RPC_path_shortcut_next:
...
.. _baking_power_overview_next:

Overview
--------

At the end of :ref:`cycle<def_cycle_next>` ``n`` (that is, the
...
:ref:`requirements<minimal_baking_power_next>`. (``CONSENSUS_RIGHTS_DELAY
= 2`` is a :ref:`protocol constant<cs_constants_next>`.)
...
account<user_accounts_next>` that has registered as a delegate by
emitting a self-``delegation`` :ref:`manager
operation<manager_operations_next>`. The list of all registered
...
emitting a :ref:`stake operation<staked_funds_management_next>`. This
...
be :ref:`slashed<slashing_next>` if the delegate misbehaves. That's
...
- The **unstaked balance** is the sum of tez contained in **unstake
  ...
  :ref:`staked_funds_management_next`. Unstake requests can be
...
- The **frozen bonds** are a deposit for :ref:`rollup
  commitments<commitments_next>`. They can be queried with RPC
...
least four cycles<staked_funds_management_next>`, so short-duration
...
parameter<staking_policy_configuration_next>` that limits how much
...
:ref:`min_delegated_in_current_cycle<min_delegated_next>`, and any
:ref:`overstaked<overstaking_next>` tez count as delegated
too. Therefore:
...
:ref:`as defined above<baking_power_overview_next>`:
...
- The delegate must be :ref:`active<active_delegate_next>`

where ``MINIMAL_STAKE = 6,000ꜩ`` and ``MINIMAL_FROZEN_STAKE = 600ꜩ``
are :ref:`protocol constants<cs_constants_next>`.
