...
:ref:`slots<slots_r022>`, called **baking rights**, to each
:ref:`delegate a.k.a. baker<def_delegate_r022>`. For this selection
process, each baker is weighted according to its **baking power** --
provided that it is :ref:`active<active_delegate_r022>` and meets the
:ref:`minimal power and own staked
requirements<minimal_baking_power_r022>`.
...
each delegate's :ref:`voting power<voting_power_r022>` instead, which
is computed in a similar but simpler way.


.. _RPC_path_shortcut_r022:
...
.. _baking_power_overview_r022:

Overview
--------

At the end of :ref:`cycle<def_cycle_r022>` ``n`` (that is, the
...
:ref:`requirements<minimal_baking_power_r022>`. (``CONSENSUS_RIGHTS_DELAY
= 2`` is a :ref:`protocol constant<cs_constants_r022>`.)
...
account<user_accounts_r022>` that has registered as a delegate by
emitting a self-``delegation`` :ref:`manager
operation<manager_operations_r022>`. The list of all registered
...
emitting a :ref:`stake operation<staked_funds_management_r022>`. This
...
be :ref:`slashed<slashing_r022>` if the delegate misbehaves. That's
...
- The **unstaked balance** is the sum of tez contained in **unstake
  ...
  :ref:`staked_funds_management_r022`. Unstake requests can be
...
- The **frozen bonds** are a deposit for :ref:`rollup
  commitments<commitments_r022>`. They can be queried with RPC
...
least four cycles<staked_funds_management_r022>`, so short-duration
...
parameter<staking_policy_configuration_r022>` that limits how much
...
:ref:`min_delegated_in_current_cycle<min_delegated_r022>`, and any
:ref:`overstaked<overstaking_r022>` tez count as delegated
too. Therefore:
...
:ref:`as defined above<baking_power_overview_r022>`:
...
- The delegate must be :ref:`active<active_delegate_r022>`

where ``MINIMAL_STAKE = 6,000ꜩ`` and ``MINIMAL_FROZEN_STAKE = 600ꜩ``
are :ref:`protocol constants<cs_constants_r022>`.
