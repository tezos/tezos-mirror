...
parser.add_argument("proto_dir",
                    help="a protocol directory containing its documentation")
...
if not os.path.isdir(dir):
    print(f"directory {dir} not found", file = sys.stderr)
    exit(1)

# First pass: Scan labels: check & register
def_lbl_pat = re.compile(r' *[.][.]  *_([a-zA-Z0-9_-]*) *:')
for file in glob.glob(f"{dir}/*.rst"):
        ...
        while line := f.readline():
            if m := re.match(def_lbl_pat, line):
                ...
                if args.labels and not label.endswith('_' + proto):
                    print(f"{file}: unversioned label {label} found, please remove or rewrite", file = sys.stderr)
...
for file in glob.glob(f"{dir}/*.rst"):
    ...
    for i in re.finditer(ref_pat, txt):
        ...
        if not (m := re.fullmatch(sphinx_id_pat, label)):
            print(f"{file}: :ref:`{ref}`: rewrite as ref to a label",
                    file = sys.stderr)
            continue
        ...
        lclabel = label.lower()
        # external ref ok:
        if not lclabel in labels and not f"lclabel_{proto}" in labels:
            continue
        print(f"{file}: :ref:`{ref}`: label {label} should be versioned as {label}_{proto}",
            file = sys.stderr)
    # end for ref
