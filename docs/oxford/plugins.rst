...
:src:`src/proto_018_Proxford/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_oxford`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_oxford`).

.. _prevalidator_filters_oxford:
...
on the :ref:`validation pass<validation_passes_oxford>`.Some extra information (like the fees, or the gas/fees
