...
A coin flip contract on Tezos source code :src:`<src/proto_018_Proxford/lib_protocol/test/integration/michelson/contracts/timelock_flip.tz>` gives an example of using time-lock. Beware, this contract is for educational purpose only, and is not secure.
