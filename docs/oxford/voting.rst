...
has a voting power equal to its *stake*. More precisely, the voting power of a delegate during a voting period is its :ref:`staking balance <active_stake_oxford>`, measured in *mutez*, sampled at the beginning of the period. (Note that this is opposed to validator selection for consensus, which is based on the active stake, and sampled at some stake snapshot during some previous cycle.)
...
:src:`src/proto_018_Proxford/lib_protocol/amendment.ml`.
...
:src:`src/proto_018_Proxford/lib_protocol/operation_repr.ml`.
