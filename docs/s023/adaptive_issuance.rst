...
At the end of each blockchain :ref:`cycle <def_cycle_s023>`, the
...
adaptive issuance rate is the sum of a :ref:`static rate <static_rate_s023>`
and a :ref:`dynamic rate <dynamic_rate_s023>`. This value is kept within
...
- ``issuance_modification_delay`` is a :ref:`derived protocol
  constant<protocol_constants_s023>` which is set to the same value
  as ``CONSENSUS_RIGHTS_DELAY`` (see :ref:`active_stake_s023`), that
  is, 2 cycles.
...
.. note::
  ...
  rate<minimum_and_maximum_rates_s023>` is reached, it is possible,
...
<minimum_and_maximum_rates_s023>`, along with the :ref:`adaptive
maximum <adaptive_maximum_s023>`, computed for the cycle ``c + 1``.
...
<rewards_weights_s023>`.

.. _rewards_weights_s023:
...
for contributing to :ref:`random seed generation <randomness_generation_s023>`
