...
From our latest bench the time to validate the migration block on the
...
This is due to our current storage backend that will be replaced in the
`near future <https://tarides.com/blog/2019-05-13-on-the-road-to-irmin-v2.html>`_.
...
Here is an example migration from Mainnet April to a node in mode
`full`. Assuming a running node from Mainnet April with default
...
However we can also specify the block to snapshot::

  tezos-node snapshot export -b $HASH mainnet-$HASH.full

We can now delete our upgraded archive storage and import the snapshot
(without forgetting the node identity)::

    cp ~/tezos-node/identity.json ~/
    rm -r ~/.tezos-node
...
Currently the import requires 8GB of ram. This will be
significantly reduced in the next release.
If you have a small vps with less memory, one possible workaround is to
import the snapshot on a machine with more ram and then copy the
directory `.tezos-node` on the vps.
...
- Commits:
    ...
    - ``52b196a72``: ``P2p: Fix error message from improve cancelation``
...
- Commits:
    - ``a3379065e``: ``Distributed_db: fix concurency issue on Peer_validator creation``
    ...
    - ``aa02c8580``: ``Shell: delete block when cuting alternative heads``
...
- better support for user interuption (CTRL-C), especially when forking test chain
...
- history modes
- add ``show current checkopoint`` command to admin client
...
- Commits:
    ...
    - ``3967e43b7``: ``sandboxed-node: preverse context if DATA_DIR variable is defined``
    ...
    - ``2d8e8b17d``: ``Daemons: restore nonces file retrocompatibility``
    - ``57f17e50c``: ``Daemons: restore highwatermarks file retrocompatibility``
...
- Commits:
    ...
    - ``82e06dab9``: ``Tests: add OCaml framewwork flextesa``
    ...
    - ``4511710d4``: ``Flextesa: voting commmand takes the TEZOS_PROTOCOL file as argument``
