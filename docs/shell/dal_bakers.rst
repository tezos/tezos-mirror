...
- If a slot commitment is included in an L1 block at some level ``PL`` (a.k.a ``published_level``), the baker can attest the slot's shards at any of the levels ``PL + lag`` for each ``lag`` in ``attestation_lags``. The corresponding attestation operation is injected on top of the block at level ``PL + lag - 1``. A single attestation at a given level covers all applicable published levels simultaneously.
...
- If bakers use a tz4 :ref:`consensus key <consensus_key_details>` (or don't have a consensus key and use a tz4 delegate key), they need to register and use a :ref:`companion key <companion_key>` in order to include their DAL payload in their attestations. See :ref:`the key management guide <consensus_companion_example>` for a complete setup walkthrough.
