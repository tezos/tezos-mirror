...
complementary to the existing :ref:`delegate <def_delegate_tallinn>`
...
subject to :ref:`slashing <slashing_tallinn>` if
the delegate misbehaves by :ref:`double-signing <def_double_signing_tallinn>`
...
:ref:`Participation rewards <adaptive_rewards_tallinn>` are automatically shared
...
policy <staking_policy_configuration_tallinn>` by setting staking parameters
...
the same name introduced for :ref:`user accounts <def_user_account_tallinn>`.
...
another :ref:`UNSTAKE_FINALIZATION_DELAY<cs_constants_tallinn>` full
