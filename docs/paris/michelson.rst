...
the :ref:`concrete syntax <ConcreteSyntax_paris>`. In particular
...
:ref:`special annotations <SpecialAnnotations_paris>` and are not allowed everywhere.
...
The multisig commands of :ref:`Octez command line client <client_manual_paris>`
use this
...
:ref:`similar to Michelson scripts <syntax_of_scripts_paris>` but
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_paris>`).
...
:ref:`syntax of errors <syntax_of_errors_paris>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_paris>` can
...
particular order.
 ...
 - ``other_contracts`` (optional, defaults to ``{}``): mapping between
   ...
   specifications <syntax_of_other_contracts_paris>`)
 ...
 - ``big_maps`` (optional, defaults to ``{}``): mapping between
   ...
   <syntax_of_extra_big_maps_paris>`)
...
<syntax_of_concrete_operations_paris>`) or implementation-specific
...
:ref:`wildcard pattern <omitting_parts_of_the_output_paris>` in the
...
raise:
 ...
 - ``(StaticError <error description>)``: an error occurred before the
   ...
   <omitting_parts_of_the_output_paris>` such as ``(StaticError _)``
...
<omitting_parts_of_the_output_paris>`.
