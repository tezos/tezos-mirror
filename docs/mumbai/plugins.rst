...
:src:`src/proto_016_PtMumbai/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_mumbai`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_mumbai`).

.. _prevalidator_filters:
.. _prevalidator_filters_mumbai:
...
:ref:`filter RPCs<active_filter_rpc_mumbai>`) parameters of the filter
...
on the :ref:`validation pass<validation_passes_mumbai>`.Some extra information (like the fees, or the gas/fees
