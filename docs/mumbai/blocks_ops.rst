...
delegates which :ref:`"double sign" <def_double_signing_mumbai>` blocks, or emit
...
Tezos fundraiser to make their :ref:`accounts <def_account_mumbai>` operational.
...
:ref:`fees<def_fee_mumbai>` to the baker for its inclusion in a block. Indeed,
manager operations are the only fee-paying and
:ref:`gas-consuming<def_gas_mumbai>` operations.
...
- The ``Delegation`` operation allows users to :ref:`delegate their
  stake <delegating_coins>` to a :ref:`delegate<def_delegate_mumbai>` (a
...
- The ``Origination`` operation is used to
  :ref:`originate<def_origination_mumbai>`, that is to deploy, smart contracts
