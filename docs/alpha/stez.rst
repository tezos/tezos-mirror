...
freeze tez towards a :ref:`delegate's <def_delegate_alpha>` security
deposit to earn :ref:`participation rewards <adaptive_rewards_alpha>`,
...
value accrues through an :ref:`exchange rate <exchange_rate_alpha>`,
...
<stez_staking_power_alpha>` across registered delegates each cycle.
...
<adaptive_rewards_alpha>` and :doc:`slashing <adaptive_slashing>` for
...
<exchange_rate_alpha>`. In :ref:`receipts <stez_receipts_alpha>`, this
...
:ref:`Finalizing a Redemption <stez_user_operations_alpha>`). In
:ref:`receipts <stez_receipts_alpha>`, this ledger appears as
``stez_redeemed_deposits``.

.. _exchange_rate_alpha:
...
staking ledger <stez_staking_ledger_alpha>` and ``total_supply`` is the total number of sTEZ tokens
...
across :ref:`registered <stez_delegate_operations_alpha>`
...
- **Delegate allocation maximum** (``GLOBAL_STEZ_BAKER_CAPACITY``):
  ...
  external stake <stez_staking_power_alpha>` that can be filled by
...
uses :ref:`pseudo-operations <pseudo_operations_alpha>`
...
ledger <stez_staking_ledger_alpha>`
and mints an equivalent value of sTEZ tokens (based on the current
:ref:`exchange rate <exchange_rate_alpha>`), which are fully
transferable.
...
<exchange_rate_alpha>`) from the :ref:`sTEZ staking ledger
<stez_staking_ledger_alpha>` to the :ref:`sTEZ Unstaked Frozen Ledger
<stez_staking_ledger_alpha>`. The amount is specified in **sTEZ
tokens** (not tez).
...
<staked_funds_management_alpha>`). During this period, the funds may
...
.. _stez_delegate_operations_alpha:
...
<stez_staking_ledger_alpha>`. Delegates
...
:ref:`Protocol Rules and Constants <stez_safeguards_alpha>`.
...
- **Capacity**: the maximum proportion of the delegate's maximum
  ...
  <stez_staking_power_alpha>` for how capacity is used in the
  allocation.
...
- ``stez_deposits`` -- the global sTEZ staking ledger (frozen tez
  ...
  <exchange_rate_alpha>` and debits (from slashing) decrease it.
