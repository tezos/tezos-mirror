...
- Every operation contained are :ref:`valid<operation_validity_ushuaia>`;

- every operation are :ref:`co-valid<co-valid_operations_ushuaia>`: they can be
...
mode<partial_construction_ushuaia>` to incrementally validate new operations while
maintaining the aforementioned invariants.

During validation, operations are never actually applied, as it is unnecessary
for asserting their :ref:`validity<operation_validity_ushuaia>`.
