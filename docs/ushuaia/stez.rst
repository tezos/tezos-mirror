...
freeze tez towards a :ref:`delegate's <def_delegate_ushuaia>` security
deposit to earn :ref:`participation rewards <adaptive_rewards_ushuaia>`,
...
value accrues through an :ref:`exchange rate <exchange_rate_ushuaia>`,
...
<stez_staking_power_ushuaia>` across registered delegates each cycle.
...
:ref:`internal ledgers <stez_staking_ledger_ushuaia>` that track funds,
the :ref:`exchange rate <exchange_rate_ushuaia>` that determines token
value, the :ref:`fee auction <stez_global_fee_ushuaia>` that determines
delegate fees, the :ref:`allocation algorithm
<stez_staking_power_ushuaia>` that distributes staking power, and the
:ref:`rules and constants <stez_safeguards_ushuaia>` that govern the
system.

.. _stez_staking_ledger:
.. _stez_staking_ledger_ushuaia:
...
<adaptive_rewards_ushuaia>` and :doc:`slashing <adaptive_slashing>` for
...
<exchange_rate_ushuaia>`. In :ref:`receipts <stez_receipts_ushuaia>`, this
...
:ref:`Finalizing a Redemption <stez_user_operations_ushuaia>`). In
:ref:`receipts <stez_receipts_ushuaia>`, this ledger appears as
``stez_redeemed_deposits``.

.. _exchange_rate:
.. _exchange_rate_ushuaia:
...
staking ledger <stez_staking_ledger_ushuaia>` and ``total_supply`` is the total number of sTEZ tokens
...
<stez_staking_ledger_ushuaia>`,
increasing the :ref:`exchange rate <exchange_rate_ushuaia>` for all sTEZ
holders.
...
<adaptive_rewards_ushuaia>` are computed and distributed, see
:doc:`adaptive_issuance`.

.. _stez_staking_power:
.. _stez_staking_power_ushuaia:

Staking Power Distribution Across Delegates
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

At the end of each cycle, the protocol allocates sTEZ staking power
across :ref:`registered <stez_delegate_operations_ushuaia>`
...
- **Delegate allocation maximum** (``GLOBAL_STEZ_BAKER_CAPACITY``):
  ...
  external stake <stez_staking_power_ushuaia>` that can be filled by
...
- **Delegate fee maximum** (``GLOBAL_STEZ_BAKER_EDGE``): the maximum
  ...
  <stez_staking_ledger_ushuaia>`. See :ref:`Global Fee
  <stez_global_fee_ushuaia>` for how bids determine delegate selection
...
uses :ref:`pseudo-operations <pseudo_operations_ushuaia>`
...
ledger <stez_staking_ledger_ushuaia>`
and mints an equivalent value of sTEZ tokens (based on the current
:ref:`exchange rate <exchange_rate_ushuaia>`), which are fully
transferable.
...
<exchange_rate_ushuaia>`) from the :ref:`sTEZ staking ledger
<stez_staking_ledger_ushuaia>` to the :ref:`sTEZ Unstaked Frozen Ledger
<stez_staking_ledger_ushuaia>`. The amount is specified in **sTEZ
tokens** (not tez).
...
<staked_funds_management_ushuaia>`). During this period, the funds may
...
.. _stez_delegate_operations_ushuaia:
...
<stez_staking_ledger_ushuaia>`. Delegates
...
:ref:`Protocol Rules and Constants <stez_safeguards_ushuaia>`.
...
- **Capacity**: the maximum proportion of the delegate's maximum
  ...
  <stez_staking_power_ushuaia>` for how capacity is used in the
  allocation.
...
- ``stez_deposits`` -- the global sTEZ staking ledger (frozen tez
  ...
  <exchange_rate_ushuaia>` and debits (from slashing) decrease it.
