...
At the end of each blockchain :ref:`cycle <def_cycle_ushuaia>`, the
...
.. _adaptive_issuance_rate_ushuaia:
...
adaptive issuance rate is the sum of a :ref:`static rate <static_rate_ushuaia>`
and a :ref:`dynamic rate <dynamic_rate_ushuaia>`. This value is kept within
a minimal and a maximal value, to ensure nominal emissions remain within
reasonable bounds.

.. _staked_ratio:
.. _staked_ratio_ushuaia:
...
- ``issuance_modification_delay`` is a :ref:`derived protocol
  constant<protocol_constants_ushuaia>` which is set to the same value
  as ``CONSENSUS_RIGHTS_DELAY`` (see :ref:`active_stake_ushuaia`), that
  is, 2 cycles.
...
.. note::
  ...
  rate<minimum_and_maximum_rates_ushuaia>` is reached, it is possible,
...
<minimum_and_maximum_rates_ushuaia>`, along with the :ref:`adaptive
maximum <adaptive_maximum_ushuaia>`, computed for the cycle ``c + 1``.
...
<rewards_weights_ushuaia>`.

.. _rewards_weights:
.. _rewards_weights_ushuaia:
...
for contributing to :ref:`random seed generation <randomness_generation_ushuaia>`
...
The `RPC endpoint
<./rpc.html#get-block-id-context-issuance-expected-issuance>`__,
