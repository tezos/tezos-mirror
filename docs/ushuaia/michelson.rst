...
.. _transaction_semantics_ushuaia:
...
Note that :ref:`manager operations batches <manager_operations_batches_ushuaia>` contain a sequence of external operations signed as a whole by a source user account, which are executed atomically.
...
the :ref:`concrete syntax <ConcreteSyntax_ushuaia>`. In particular
...
.. _michelson_type_system_ushuaia:
...
However, one side effect of this RPC is the stripping of :ref:`annotations <annotations_ushuaia>`.
...
Note that all annotations are stripped during :ref:`type normalization <type_normalization_ushuaia>`.
...
:ref:`special annotations <SpecialAnnotations_ushuaia>` and are not allowed everywhere.
...
The multisig commands of :ref:`Octez command line client <client_manual_ushuaia>`
use this
...
:ref:`similar to Michelson scripts <syntax_of_scripts_ushuaia>` but
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_ushuaia>`).
...
:ref:`syntax of errors <syntax_of_errors_ushuaia>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_ushuaia>` can
...
- ``other_contracts`` (optional, defaults to ``{}``): mapping between
  ...
  specifications <syntax_of_other_contracts_ushuaia>`)
...
.. _syntax_of_concrete_stacks_ushuaia:
...
.. _omitting_parts_of_the_output_ushuaia:
...
<syntax_of_concrete_operations_ushuaia>`) or implementation-specific
parts of error outputs (see the :ref:`syntax of errors
<syntax_of_errors_ushuaia>`).

.. _output_normalization:
.. _output_normalization_ushuaia:
...
:ref:`wildcard pattern <omitting_parts_of_the_output_ushuaia>` in the
...
- ``(StaticError <error description>)``: an error occurred before the
  ...
  <omitting_parts_of_the_output_ushuaia>` such as ``(StaticError _)``
...
.. _syntax_of_concrete_operations_ushuaia:
...
<omitting_parts_of_the_output_ushuaia>`.
...
.. _syntax_of_other_contracts_ushuaia:
...
.. _syntax_of_extra_big_maps_ushuaia:
