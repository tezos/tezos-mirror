...
complementary to the existing :ref:`delegate <def_delegate_ushuaia>`
...
subject to :ref:`slashing <slashing_ushuaia>` if
the delegate misbehaves by :ref:`double-signing <def_double_signing_ushuaia>`
...
:ref:`Participation rewards <adaptive_rewards_ushuaia>` are automatically shared
...
policy <staking_policy_configuration_ushuaia>` by setting staking parameters
...
management <staked_funds_management_ushuaia>`).

.. _pseudo_operations:
.. _pseudo_operations_ushuaia:
...
the same name introduced for :ref:`user accounts <def_user_account_ushuaia>`.
...
.. _staking_policy_configuration_ushuaia:
...
.. _staked_funds_management_ushuaia:
...
another :ref:`UNSTAKE_FINALIZATION_DELAY<cs_constants_ushuaia>` full
