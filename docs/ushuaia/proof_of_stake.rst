...
<staking_coins>`. Indeed, in Tezos, it is the :ref:`delegates<def_delegate_ushuaia>`
...
The PRNG's :ref:`seeds <random_seed_ushuaia>` are obtained from random
...
A *delegate* is any :ref:`user account <def_user_account_ushuaia>` registered as
...
Any :ref:`account <def_account_ushuaia>` (user account or smart contract) can specify a delegate
...
becomes effective after ``CONSENSUS_RIGHTS_DELAY + 2`` :ref:`cycles <def_cycle_ushuaia>`.  The
value ``CONSENSUS_RIGHTS_DELAY`` is a :ref:`protocol constant
<protocol_constants_ushuaia>`. A delegate cannot stop self-delegating.
...
needs to be :ref:`active<active_delegate_ushuaia>` and to meet
:ref:`minimal balance requirements<minimal_baking_power_ushuaia>`.

.. _security_deposit:
.. _security_deposit_ushuaia:
...
another :ref:`CONSENSUS_KEY_ACTIVATION_DELAY<cs_constants_ushuaia>` full
...
the :ref:`stakes<active_stake_ushuaia>` of a specific cycle: the probability to sample a
...
:ref:`random seed<random_seed_ushuaia>` for the cycle to which the
level belongs, and the slot.


.. _ps_constants:
.. _ps_constants_ushuaia:
...
The adaptive issuance feature :ref:`documentation <adaptive_issuance_ushuaia>`.
