...
_`Double signing`
    ...
    part of the double signer's deposit -- see :ref:`Slashing<slashing_ushuaia>`.

_`Failing Noop`
    The ``Failing_noop`` operation implements a *No-op*, which always
    fails at :ref:`application time<operation_validity_ushuaia>`, and
    should never appear in :ref:`applied
    blocks<full_application_ushuaia>`. This operation allows end-users to
    :ref:`sign arbitrary messages<failing_noop_ushuaia>` which have no
...
_`Fee`
    ...
    Currently, only :ref:`manager operations<manager_operations_ushuaia>`
...
_`Staker`
    ...
    subject to slashing in case the delegate misbehaves -- see :ref:`Slashing<slashing_ushuaia>`.
...
_`Validation pass`
    ...
    application<operation_validity_ushuaia>` of certain classes of
    operations.
...
_`Voting power`
    ...
    it. See :ref:`voting_power_ushuaia` for details.
