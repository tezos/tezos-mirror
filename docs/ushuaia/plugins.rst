...
:src:`src/proto_025_PsUshuai/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_ushuaia`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_ushuaia`).

.. _prevalidator_filters:
.. _prevalidator_filters_ushuaia:
...
:ref:`filter RPCs<active_filter_rpc_ushuaia>`) parameters of the filter
configuration of your node.

.. _consensus_filter:
.. _consensus_filter_ushuaia:
...
RPCs<active_filter_rpc_ushuaia>`) parameter of the filter configuration of your
node.
...
pass<validation_passes_ushuaia>`. Some extra information (like the fees, or the
