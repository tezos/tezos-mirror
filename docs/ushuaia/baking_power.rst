...
:ref:`slots<slots_ushuaia>`, called **baking rights**, to each
:ref:`delegate a.k.a. baker<def_delegate_ushuaia>`. For this selection
process, each baker is weighted according to its **baking power** --
provided that it is :ref:`active<active_delegate_ushuaia>` and meets the
:ref:`minimal power and own staked
requirements<minimal_baking_power_ushuaia>`.
...
each delegate's :ref:`voting power<voting_power_ushuaia>` instead, which
is computed in a similar but simpler way.


.. _RPC_path_shortcut:
.. _RPC_path_shortcut_ushuaia:
...
.. _baking_power_overview_ushuaia:

Overview
--------

At the end of :ref:`cycle<def_cycle_ushuaia>` ``n`` (that is, the
...
:ref:`requirements<minimal_baking_power_ushuaia>`. (``CONSENSUS_RIGHTS_DELAY
= 2`` is a :ref:`protocol constant<cs_constants_ushuaia>`.)
...
account<user_accounts_ushuaia>` that has registered as a delegate by
emitting a self-``delegation`` :ref:`manager
operation<manager_operations_ushuaia>`. The list of all registered
...
emitting a :ref:`stake operation<staked_funds_management_ushuaia>`. This
...
be :ref:`slashed<slashing_ushuaia>` if the delegate misbehaves. That's
...
- The **unstaked balance** is the sum of the tez amounts contained in
  ...
  :ref:`staked_funds_management_ushuaia`. Unstake requests can be
...
- The **frozen bonds** are a deposit for :ref:`rollup
  commitments<commitments_ushuaia>`. They can be queried with RPC
...
least four cycles<staked_funds_management_ushuaia>`, so short-duration
...
.. _overstaking:
.. _overstaking_ushuaia:

Overstaking
-----------

The **limit_of_staking_over_baking** is a :ref:`configurable delegate
parameter<staking_policy_configuration_ushuaia>` that limits how much
...
:ref:`min_delegated_in_current_cycle<min_delegated_ushuaia>`, and any
:ref:`overstaked<overstaking_ushuaia>` tez count as delegated
too. Therefore:
...
:ref:`as defined above<baking_power_overview_ushuaia>`:
...
- The delegate must be :ref:`active<active_delegate_ushuaia>`

where ``MINIMAL_STAKE = 6,000ꜩ`` and ``MINIMAL_FROZEN_STAKE = 600ꜩ``
are :ref:`protocol constants<cs_constants_ushuaia>`.
