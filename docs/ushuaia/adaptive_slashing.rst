...
function<adaptive_slashing_fn_ushuaia>` implementing this idea, as well as the
:ref:`new forbidden period<new_forbidden_period_ushuaia>`.

.. _adaptive_slashing_fn:
.. _adaptive_slashing_fn_ushuaia:

Adaptive Slashing Function
==========================

.. _adaptive_slashing_informal:
.. _adaptive_slashing_informal_ushuaia:
...
.. _formal_adaptive_slashing_ushuaia:
...
* :math:`\mathcal{W}` denotes the maximal possible *weight* of
  ...
  :ref:`slots<rights_ushuaia>` in any block. It is also known as
  :ref:`CONSENSUS_COMMITTEE_SIZE<tb_validator_ushuaia>`.
...
* :math:`T` is the *threshold* for the fraction of double attestations
  ...
  :ref:`CONSENSUS_THRESHOLD<tb_validator_ushuaia>` which is set to
...
* the current total frozen stake for the delegate (sum of the
  :ref:`staking balances<active_stake_ushuaia>` of the delegate itself
  and its stakers) is at least as high as the :ref:`active
  stake<active_stake_ushuaia>` that was used ``CONSENSUS_RIGHTS_DELAY``
