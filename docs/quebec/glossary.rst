...
_`Double signing`
    ...
    part of the double signer's deposit -- see :ref:`Slashing<slashing_qena>`.

_`Failing Noop`
    The ``Failing_noop`` operation implements a *No-op*, which always
    fails at :ref:`application time<operation_validity_qena>`, and
    should never appear in :ref:`applied
    blocks<full_application_qena>`. This operation allows end-users to
    :ref:`sign arbitrary messages<failing_noop_qena>` which have no
...
_`Fee`
    ...
    Currently, only :ref:`manager operations<manager_operations_qena>`
...
_`Stake`
    ...
    :ref:`here<active_stake_qena>` for details.
...
_`Staker`
    ...
    subject to slashing in case the delegate misbehaves -- see :ref:`Slashing<slashing_qena>`.
...
_`Validation pass`
    ...
    application<operation_validity_qena>` of certain classes of
    operations.
