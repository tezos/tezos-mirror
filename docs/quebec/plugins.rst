...
:src:`src/proto_021_PtQuebec/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_quebec`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_quebec`).

.. _prevalidator_filters_quebec:
...
on the :ref:`validation pass<validation_passes_quebec>`.Some extra information (like the fees, or the gas/fees
