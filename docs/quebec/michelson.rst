...
Note that :ref:`manager operations batches <manager_operations_batches_qena>` contain a sequence of external operations signed as a whole by a source user account, which are executed atomically.
...
the :ref:`concrete syntax <ConcreteSyntax_qena>`. In particular
...
However, one side effect of this RPC is the stripping of :ref:`annotations <annotations_qena>`.
...
Note that all annotations are stripped during :ref:`type normalization <type_normalization_qena>`.
...
:ref:`special annotations <SpecialAnnotations_qena>` and are not allowed everywhere.
...
The multisig commands of :ref:`Octez command line client <client_manual_qena>`
use this
...
:ref:`similar to Michelson scripts <syntax_of_scripts_qena>` but
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_qena>`).
...
:ref:`syntax of errors <syntax_of_errors_qena>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_qena>` can
...
- ``other_contracts`` (optional, defaults to ``{}``): mapping between
  ...
  specifications <syntax_of_other_contracts_qena>`)
...
.. _syntax_of_concrete_stacks_qena:
...
.. _omitting_parts_of_the_output_qena:
...
<syntax_of_concrete_operations_qena>`) or implementation-specific
parts of error outputs (see the :ref:`syntax of errors
<syntax_of_errors_qena>`).

.. _output_normalization_qena:
...
:ref:`wildcard pattern <omitting_parts_of_the_output_qena>` in the
...
- ``(StaticError <error description>)``: an error occurred before the
  ...
  <omitting_parts_of_the_output_qena>` such as ``(StaticError _)``
...
.. _syntax_of_concrete_operations_qena:
...
<omitting_parts_of_the_output_qena>`.
...
.. _syntax_of_other_contracts_qena:
...
.. _syntax_of_extra_big_maps_qena:
