...
:src:`src/proto_024_PsU87LFi/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_t024`);
...
:ref:`filter RPCs<active_filter_rpc_t024>`) parameters of the filter
configuration of your node.

.. _consensus_filter_t024:
...
RPCs<active_filter_rpc_t024>`) parameter of the filter configuration of your
node.
...
pass<validation_passes_t024>`. Some extra information (like the fees, or the
