...
function<adaptive_slashing_fn_t024>` implementing this idea, as well as the
:ref:`new forbidden period<new_forbidden_period_t024>`.

.. _adaptive_slashing_fn_t024:

Adaptive Slashing Function
==========================

.. _adaptive_slashing_informal_t024:
...
.. _formal_adaptive_slashing_t024:
...
* :math:`\mathcal{W}` denotes the maximal possible *weight* of
  ...
  :ref:`slots<rights_t024>` in any block. It is also known as
  :ref:`CONSENSUS_COMMITTEE_SIZE<tb_validator_t024>`.
...
* :math:`T` is the *threshold* for the fraction of double attestations
  ...
  :ref:`CONSENSUS_THRESHOLD<tb_validator_t024>` which is set to
...
* the current total frozen stake for the delegate (sum of the
  :ref:`staking balances<active_stake_t024>` of the delegate itself
  and its stakers) is at least as high as the :ref:`active
  stake<active_stake_t024>` that was used ``CONSENSUS_RIGHTS_DELAY``
