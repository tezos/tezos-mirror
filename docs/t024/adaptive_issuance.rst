...
At the end of each blockchain :ref:`cycle <def_cycle_t024>`, the
...
adaptive issuance rate is the sum of a :ref:`static rate <static_rate_t024>`
and a :ref:`dynamic rate <dynamic_rate_t024>`. This value is kept within
...
- ``issuance_modification_delay`` is a :ref:`derived protocol
  constant<protocol_constants_t024>` which is set to the same value
  as ``CONSENSUS_RIGHTS_DELAY`` (see :ref:`active_stake_t024`), that
  is, 2 cycles.
...
.. note::
  ...
  rate<minimum_and_maximum_rates_t024>` is reached, it is possible,
...
<minimum_and_maximum_rates_t024>`, along with the :ref:`adaptive
maximum <adaptive_maximum_t024>`, computed for the cycle ``c + 1``.
...
<rewards_weights_t024>`.

.. _rewards_weights_t024:
...
for contributing to :ref:`random seed generation <randomness_generation_t024>`
