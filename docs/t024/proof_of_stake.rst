...
<staking_coins>`. Indeed, in Tezos, it is the :ref:`delegates<def_delegate_t024>`
...
The PRNG's :ref:`seeds <random_seed_t024>` are obtained from random
...
A *delegate* is any :ref:`user account <def_user_account_t024>` registered as
...
Any :ref:`account <def_account_t024>` (user account or smart contract) can specify a delegate
...
becomes effective after ``CONSENSUS_RIGHTS_DELAY + 2`` :ref:`cycles <def_cycle_t024>`.  The
value ``CONSENSUS_RIGHTS_DELAY`` is a :ref:`protocol constant
<protocol_constants_t024>`. A delegate cannot stop self-delegating.
...
needs to be :ref:`active<active_delegate_t024>` and to meet
:ref:`minimal balance requirements<minimal_baking_power_t024>`.

.. _security_deposit_t024:
...
another :ref:`CONSENSUS_KEY_ACTIVATION_DELAY<cs_constants_t024>` full
...
the :ref:`stakes<active_stake_t024>` of a specific cycle: the probability to sample a
...
:ref:`random seed<random_seed_t024>` for the cycle to which the
...
The adaptive issuance feature :ref:`documentation <adaptive_issuance_t024>`.
