...
:ref:`slots<slots_t024>`, called **baking rights**, to each
:ref:`delegate a.k.a. baker<def_delegate_t024>`. For this selection
process, each baker is weighted according to its **baking power** --
provided that it is :ref:`active<active_delegate_t024>` and meets the
...
each delegate's :ref:`voting power<voting_power_t024>` instead, which
is computed in a similar but simpler way.


.. _RPC_path_shortcut_t024:
...
.. _baking_power_overview_t024:

Overview
--------

At the end of :ref:`cycle<def_cycle_t024>` ``n`` (that is, the
...
:ref:`requirements<minimal_baking_power_t024>`. (``CONSENSUS_RIGHTS_DELAY
= 2`` is a :ref:`protocol constant<cs_constants_t024>`.)
...
account<user_accounts_t024>` that has registered as a delegate by
emitting a self-``delegation`` :ref:`manager
operation<manager_operations_t024>`. The list of all registered
...
emitting a :ref:`stake operation<staked_funds_management_t024>`. This
...
be :ref:`slashed<slashing_t024>` if the delegate misbehaves. That's
...
- The **unstaked balance** is the sum of the tez amounts contained in
  ...
  :ref:`staked_funds_management_t024`. Unstake requests can be
...
- The **frozen bonds** are a deposit for :ref:`rollup
  commitments<commitments_t024>`. They can be queried with RPC
...
least four cycles<staked_funds_management_t024>`, so short-duration
...
parameter<staking_policy_configuration_t024>` that limits how much
...
:ref:`min_delegated_in_current_cycle<min_delegated_t024>`, and any
:ref:`overstaked<overstaking_t024>` tez count as delegated
too. Therefore:
...
:ref:`as defined above<baking_power_overview_t024>`:
...
- The delegate must be :ref:`active<active_delegate_t024>`

where ``MINIMAL_STAKE = 6,000ꜩ`` and ``MINIMAL_FROZEN_STAKE = 600ꜩ``
are :ref:`protocol constants<cs_constants_t024>`.
