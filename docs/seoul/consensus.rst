...
As soon as a delegate is denounced for any double signing, it is immediately :ref:`forbidden<new_forbidden_period_seoul>` from both baking and attesting for at least 2 cycles.
...
For a technical explanation of the periods when a slashed baker is unable to bake (limited to an event when the Rio protocol was active), see :download:`Forbidden periods for slashed bakers <https://octez.tezos.com/archive/SlashingIncidentExplanation.pdf>`.
