...
:ref:`slots<slots_s023>`, called **baking rights**, to each
:ref:`delegate a.k.a. baker<def_delegate_s023>`. For this selection
process, each baker is weighted according to its **baking power** --
provided that it is :ref:`active<active_delegate_s023>` and meets the
...
each delegate's :ref:`voting power<voting_power_s023>` instead, which
is computed in a similar but simpler way.


.. _RPC_path_shortcut_s023:
...
.. _baking_power_overview_s023:

Overview
--------

At the end of :ref:`cycle<def_cycle_s023>` ``n`` (that is, the
...
:ref:`requirements<minimal_baking_power_s023>`. (``CONSENSUS_RIGHTS_DELAY
= 2`` is a :ref:`protocol constant<cs_constants_s023>`.)
...
account<user_accounts_s023>` that has registered as a delegate by
emitting a self-``delegation`` :ref:`manager
operation<manager_operations_s023>`. The list of all registered
...
emitting a :ref:`stake operation<staked_funds_management_s023>`. This
...
be :ref:`slashed<slashing_s023>` if the delegate misbehaves. That's
...
- The **unstaked balance** is the sum of the tez amounts contained in
  ...
  :ref:`staked_funds_management_s023`. Unstake requests can be
...
- The **frozen bonds** are a deposit for :ref:`rollup
  commitments<commitments_s023>`. They can be queried with RPC
...
least four cycles<staked_funds_management_s023>`, so short-duration
...
parameter<staking_policy_configuration_s023>` that limits how much
...
:ref:`min_delegated_in_current_cycle<min_delegated_s023>`, and any
:ref:`overstaked<overstaking_s023>` tez count as delegated
too. Therefore:
...
:ref:`as defined above<baking_power_overview_s023>`:
...
- The delegate must be :ref:`active<active_delegate_s023>`

where ``MINIMAL_STAKE = 6,000ꜩ`` and ``MINIMAL_FROZEN_STAKE = 600ꜩ``
are :ref:`protocol constants<cs_constants_s023>`.
