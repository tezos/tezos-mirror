...
function<adaptive_slashing_fn_s023>` implementing this idea, as well as the
:ref:`new forbidden period<new_forbidden_period_s023>`.

.. _adaptive_slashing_fn_s023:

Adaptive Slashing Function
==========================

.. _adaptive_slashing_informal_s023:
...
.. _formal_adaptive_slashing_s023:
...
* :math:`\mathcal{W}` denotes the maximal possible *weight* of
  ...
  :ref:`slots<rights_s023>` in any block. It is also known as
  :ref:`CONSENSUS_COMMITTEE_SIZE<tb_validator_s023>`.
...
* :math:`T` is the *threshold* for the fraction of double attestations
  ...
  :ref:`CONSENSUS_THRESHOLD<tb_validator_s023>` which is set to
...
* the current total frozen stake for the delegate (sum of the
  :ref:`staking balances<active_stake_s023>` of the delegate itself
  and its stakers) is at least as high as the :ref:`active
  stake<active_stake_s023>` that was used ``CONSENSUS_RIGHTS_DELAY``
