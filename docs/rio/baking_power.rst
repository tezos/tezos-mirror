...
:ref:`slots<slots_rio>`, called **baking rights**, to each
:ref:`delegate a.k.a. baker<def_delegate_rio>`. For this selection
process, each baker is weighted according to its **baking power** --
provided that it is :ref:`active<active_delegate_rio>` and meets the
...
each delegate's :ref:`voting power<voting_power_rio>` instead, which
is computed in a similar but simpler way.


.. _RPC_path_shortcut:
.. _RPC_path_shortcut_rio:
...
.. _baking_power_overview_rio:

Overview
--------

At the end of :ref:`cycle<def_cycle_rio>` ``n`` (that is, the
...
:ref:`requirements<minimal_baking_power_rio>`. (``CONSENSUS_RIGHTS_DELAY
= 2`` is a :ref:`protocol constant<cs_constants_rio>`.)
...
account<user_accounts_rio>` that has registered as a delegate by
emitting a self-``delegation`` :ref:`manager
operation<manager_operations_rio>`. The list of all registered
...
emitting a :ref:`stake operation<staked_funds_management_rio>`. This
...
be :ref:`slashed<slashing_rio>` if the delegate misbehaves. That's
...
- The **unstaked balance** is the sum of the tez amounts contained in
  ...
  :ref:`staked_funds_management_rio`. Unstake requests can be
...
- The **frozen bonds** are a deposit for :ref:`rollup
  commitments<commitments_rio>`. They can be queried with RPC
...
least four cycles<staked_funds_management_rio>`, so short-duration
...
parameter<staking_policy_configuration_rio>` that limits how much
...
:ref:`min_delegated_in_current_cycle<min_delegated_rio>`, and any
:ref:`overstaked<overstaking_rio>` tez count as delegated
too. Therefore:
...
:ref:`as defined above<baking_power_overview_rio>`:
...
- The delegate must be :ref:`active<active_delegate_rio>`

where ``MINIMAL_STAKE = 6,000ꜩ`` and ``MINIMAL_FROZEN_STAKE = 600ꜩ``
are :ref:`protocol constants<cs_constants_rio>`.
