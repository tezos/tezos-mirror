...
Note that :ref:`manager operations batches <manager_operations_batches_rio>` contain a sequence of external operations signed as a whole by a source user account, which are executed atomically.
...
the :ref:`concrete syntax <ConcreteSyntax_rio>`. In particular
...
However, one side effect of this RPC is the stripping of :ref:`annotations <annotations_rio>`.
...
Note that all annotations are stripped during :ref:`type normalization <type_normalization_rio>`.
...
:ref:`special annotations <SpecialAnnotations_rio>` and are not allowed everywhere.
...
The multisig commands of :ref:`Octez command line client <client_manual_rio>`
use this
...
:ref:`similar to Michelson scripts <syntax_of_scripts_rio>` but
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_rio>`).
...
:ref:`syntax of errors <syntax_of_errors_rio>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_rio>` can
...
- ``other_contracts`` (optional, defaults to ``{}``): mapping between
  ...
  specifications <syntax_of_other_contracts_rio>`)
...
.. _syntax_of_concrete_stacks_rio:
...
.. _omitting_parts_of_the_output_rio:
...
<syntax_of_concrete_operations_rio>`) or implementation-specific
parts of error outputs (see the :ref:`syntax of errors
<syntax_of_errors_rio>`).

.. _output_normalization:
.. _output_normalization_rio:
...
:ref:`wildcard pattern <omitting_parts_of_the_output_rio>` in the
...
- ``(StaticError <error description>)``: an error occurred before the
  ...
  <omitting_parts_of_the_output_rio>` such as ``(StaticError _)``
...
.. _syntax_of_concrete_operations_rio:
...
<omitting_parts_of_the_output_rio>`.
...
.. _syntax_of_other_contracts_rio:
