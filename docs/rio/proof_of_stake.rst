...
<staking_coins>`. Indeed, in Tezos, it is the :ref:`delegates<def_delegate_rio>`
...
The PRNG's :ref:`seeds <random_seed_rio>` are obtained from random
...
A *delegate* is any :ref:`user account <def_user_account_rio>` registered as
...
Any :ref:`account <def_account_rio>` (user account or smart contract) can specify a delegate
...
becomes effective after ``CONSENSUS_RIGHTS_DELAY + 2`` :ref:`cycles <def_cycle_rio>`.  The
value ``CONSENSUS_RIGHTS_DELAY`` is a :ref:`protocol constant
<protocol_constants_rio>`. A delegate cannot stop self-delegating.
...
another :ref:`CONSENSUS_KEY_ACTIVATION_DELAY<cs_constants_rio>` full
...
the :ref:`stakes<active_stake_rio>` of a specific cycle: the probability to sample a
...
:ref:`random seed<random_seed_rio>` for the cycle to which the
...
The adaptive issuance feature :ref:`documentation <adaptive_issuance_rio>`.
