...
The language is stack-based, with high level data types and
primitives, and strict static type checking. Its design cherry picks
traits from several language families. Vigilant readers will notice
direct references to Forth, Scheme, ML and Cat.
...
first element is an input value and second element is the content of a storage
space. It must return a stack containing a single pair whose first element is
a list of internal operations, and second element is the new contents of the
...
-  ``big_map (k) (t)``: Lazily deserialized maps from keys of type
   ...
   standard maps as data is lazily deserialized.  One is limited to a
...
parameters are the manager, optional delegate, spendable flag,
delegatable flag, and the initial amount taken from the
current contract. The contract is returned as a first class
