...
:src:`src/proto_next/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_next`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_next`).

.. _prevalidator_filters_next:
...
:ref:`filter RPCs<active_filter_rpc_next>`) parameters of the filter
configuration of your node.

.. _consensus_filter_next:
...
RPCs<active_filter_rpc_next>`) parameter of the filter configuration of your
node.
...
pass<validation_passes_next>`. Some extra information (like the fees, or the
