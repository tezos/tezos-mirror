...
function<adaptive_slashing_fn_next>` implementing this idea, as well as the
:ref:`new forbidden period<new_forbidden_period_next>`.

.. _adaptive_slashing_fn_next:

Adaptive Slashing Function
==========================

.. _adaptive_slashing_informal_next:
...
.. _formal_adaptive_slashing_next:
...
* :math:`\mathcal{W}` denotes the maximal possible *weight* of
  ...
  :ref:`slots<rights_next>` in any block. It is also known as
  :ref:`CONSENSUS_COMMITTEE_SIZE<tb_validator_next>`.
...
* :math:`T` is the *threshold* for the fraction of double attestations
  ...
  :ref:`CONSENSUS_THRESHOLD<tb_validator_next>` which is set to
...
* the current total frozen stake for the delegate (sum of the
  :ref:`staking balances<active_stake_next>` of the delegate itself
  and its stakers) is at least as high as the :ref:`active
  stake<active_stake_next>` that was used ``CONSENSUS_RIGHTS_DELAY``
