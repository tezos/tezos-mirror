...
#. ``attestation``: the existing :ref:`consensus operation <consensus_operations_next>`, allowing bakers to attach a DAL payload attesting the data seen on the DAL P2P network
#. ``DAL_entrapment_evidence``: an anonymous operation to denounce a baker that has attested a trap shard

and on an :ref:`incentives scheme<DAL_incentives_scheme_next>` for the DAL.
...
As part of participation rewards, the DAL rewards are subject to the adjustments done by :ref:`Adaptive Issuance<adaptive_issuance_next>`.
...
A fixed percentage, defined by a protocol parameter called ``DAL_REWARDS_RATIO``, set to %10, of the total :ref:`participation rewards<adaptive_rewards_next>` is allocated to the DAL.

The DAL rewards per level are implicitly given by their weight, ``DAL_REWARDS_WEIGHT``, as for the other types of :ref:`participation rewards<rewards_weights_next>`.
The value of ``DAL_REWARDS_WEIGHT`` is such that it represents ``DAL_REWARDS_RATIO`` of all reward weights.

The rewards are distributed at the end of a cycle, and are computed in the same manner as for the other :ref:`participation rewards<adaptive_rewards_next>`.
...
The DAL is integrated with :doc:`smart rollups <../active/smart_rollups>` so that kernels can request pages from the DAL via the :ref:`reveal data channel <reveal_data_channel_smart_rollups_next>`. A smart rollup can fetch any page from the DAL node if the commitment respects some conditions:
...
This section describes the protocol constants specific to the DAL as well as their default values on mainnet (see :ref:`protocol_constants_next` on how to find the values for tests networks):
