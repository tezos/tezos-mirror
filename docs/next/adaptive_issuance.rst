...
At the end of each blockchain :ref:`cycle <def_cycle_next>`, the
...
adaptive issuance rate is the sum of a :ref:`static rate <static_rate_next>`
and a :ref:`dynamic rate <dynamic_rate_next>`. This value is kept within
...
- ``issuance_modification_delay`` is a :ref:`derived protocol
  constant<protocol_constants_next>` which is set to the same value
  as ``CONSENSUS_RIGHTS_DELAY`` (see :ref:`active_stake_next`), that
  is, 2 cycles.
...
.. note::
  ...
  rate<minimum_and_maximum_rates_next>` is reached, it is possible,
...
<minimum_and_maximum_rates_next>`, along with the :ref:`adaptive
maximum <adaptive_maximum_next>`, computed for the cycle ``c + 1``.
...
<rewards_weights_next>`.

.. _rewards_weights_next:
...
for contributing to :ref:`random seed generation <randomness_generation_next>`
