...
You also need to install Odoc which generates the OCaml API documentation, as explained :ref:`below <install_odoc>`.
...
If not, you can build the Octez binaries by executing in the parent directory (the repository root):
...
Once this is done, you can do ``make -C docs`` to build the documentation.
But the preferred way when working on the documentation is to do everything from witin the ``docs/`` directory:
...
The odoc sub-step of the makefile copies sources to a temporary directory under ``/tmp/octezdoc``
before running ``dune``. If that path is not writable (e.g. in a sandboxed
environment), override it with the ``TMPDOCDIR`` variable:

.. code-block:: bash

    make TMPDOCDIR=$TMPDIR/octezdoc

The documentation build from scratch takes quite long.
For the routine work of editing pages and rebuilding, you can do "light" builds, performing just some final steps of the build:

- You may omit recompiling the Octez sources, if they didn't change significantly.
- You may also omit generating the OCaml APIs with Odoc.
  For that, do ``make html`` instead of ``make``.
  This can take you down to the order of 1 min on a single CPU.
- You may also omit regenerating files included in the RST pages, such as online man pages and the RPC reference.
  For that, do ``make sphinx``.
  This can take you down to a fraction of minute on a single CPU.
  If you get errors about included files, you may have to switch once to ``make html`` as told above.
...
- ``remove_labels_without_proto.py``: removes unversioned labels in a protocol-specific page
  ...
  + can be used for "unlinking" the pages of a protocol; not needed when a new protocol is adopted, because pages of the old protocol are removed altogether
