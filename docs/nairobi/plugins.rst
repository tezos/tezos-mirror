...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_nairobi`);
...
:ref:`filter RPCs<active_filter_rpc_nairobi>`) parameters of the filter
...
on the :ref:`validation pass<validation_passes_nairobi>`.Some extra information (like the fees, or the gas/fees
