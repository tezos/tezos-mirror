...
the prefix ``sr1``
(see also the :ref:`kinds of address prefixes in Tezos <address_prefixes_nairobi>`).
...
#. **preimage requests**: The rollup can request arbitrary data of at
   ...
   :ref:`populating_the_reveal_channel_nairobi`.
...
operation (see :ref:`triggering_execution_outbox_message_nairobi`), typically
...
``read_input``
  ...
  the input is lost at the next invocation of ``kernel_run`` if it is not
  written in the durable storage. Since version ``2.0.0`` of the WASM PVM.
...
``store_has``
  ...
  path: a directory, a file, neither or both. Since version ``2.0.0`` of the
  WASM PVM.
...
``store_create``
  ...
  storage limits, that is 2GB - 1. Since the ``2.0.0-r1`` of the WASM PVM.
