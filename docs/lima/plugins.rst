...
:src:`src/proto_015_PtLimaPt/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_lima`);
...
:ref:`filter RPCs<active_filter_rpc_lima>`) parameters of the filter
...
on the :ref:`validation pass<validation_passes_lima>`.Some extra information (like the fees, or the gas/fees
