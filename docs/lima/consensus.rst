...
``CONSENSUS_COMMITTEE_SIZE`` be the number of validator :ref:`slots<rights_lima>` per level.
...
- The *frozen deposit* represents a percentage ``FROZEN_DEPOSIT_PERCENTAGE``
  ...
  :ref:`slashing_lima`).  Taking the maximum over an
...
before updating the delegates' :ref:`activity status<active_delegate_lima>`.
...
Not giving rewards in case of missing revelations is not new as it is :ref:`adapted<random_seed_lima>`
from Emmy*.
...
- ``seed_nonce_hash``: a commitment to :ref:`a random number<random_seed_lima>`, used to generate entropy on the chain
- ``proof_of_work_nonce``: a nonce used to pass a low-difficulty proof-of-work for the block, as a spam prevention measure
- ``liquidity_baking_toggle_vote``: :ref:`a vote<toggle_lima>` to continue the Liquidity Baking Subsidy, stop it, or abstain.
...
The fitness encapsulates more information than in Emmy* because Tenderbake is more complex: recall that blocks at the last level only represent :ref:`candidate blocks<finality_lima>`.
...
Furthermore, we also allow to change the predecessor block when it has a :ref:`smaller round<finality_lima>`.
...
re-proposals<quorum_lima>`, the fitness also includes, as the third
