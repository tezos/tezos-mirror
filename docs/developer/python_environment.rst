...
- ``python 3.10.13``. It is recommended to use `pyenv
  ...
  * You can use then ``pyenv install 3.10.13``, followed by:

    + ``pyenv local 3.10.13`` to use ``python 3.10.13`` only in the current directory (and its subdirectories, unless redefined)
    + ``pyenv global 3.10.13`` to set the python version to ``3.10.13`` globally
    + ``pyenv shell 3.10.13`` to use ``python 3.10.13`` only in the current shell
...
Now we can use ``pyenv`` to install Python 3.10.13 and set it as the
default version:
