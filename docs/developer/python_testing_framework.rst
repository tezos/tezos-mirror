...
- `python 3.10.9`. It is recommended to use `pyenv
  ...
  * You can use then ``pyenv install 3.10.9`` followed by:

    + ``pyenv local 3.10.9`` to use ``python 3.10.9`` only in the current directory (and its subdirectories, unless redefined)
    + ``pyenv global 3.10.9`` to set the python version to ``3.10.9`` globally
    + ``pyenv shell 3.10.9`` to use ``python 3.10.9`` only in the current shell
...
Now we can use ``pyenv`` to install Python 3.10.9 and set it as the
