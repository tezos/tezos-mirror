...
- `python 3.10.8`. It is recommended to use `pyenv
  ...
  * You can use then ``pyenv install 3.10.8`` followed by:

    + ``pyenv local 3.10.8`` to use ``python 3.10.8`` only in the current directory (and its subdirectories, unless redefined)
    + ``pyenv global 3.10.8`` to set the python version to ``3.10.8`` globally
    + ``pyenv shell 3.10.8`` to use ``python 3.10.8`` only in the current shell
...
Now we can use ``pyenv`` to install Python 3.10.8 and set it as the
