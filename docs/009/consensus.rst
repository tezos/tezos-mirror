...
header has a :ref:`shell part<Block>` (common to all protocols) and a
protocol-specific part. In Emmy+, :ref:`the protocol-specific part of the
header<emmyp_fitness_and_header>` contains, most notably, a timestamp, a
priority (a natural number), and the endorsements for the block at the previous
level. *Endorsements* are operations which can be seen as votes for a given
...
of participants is at :ref:`random<emmyp_slot_selection>`, independently for
...
- A participant always adopts the :ref:`fittest<emmyp_fitness_and_header>`, that
...
:ref:`roll<roll_pos>`, and is :ref:`active<active_delegate>`. For simplicity we
...
:ref:`here<rights>`. The mechanism behind baking slots is meant to ensure that
...
:ref:`protocol-specific header<Block>`:
...
- the protocol-specific header of a block has the following fields:
  ...
  - ``seed_nonce_hash``: a commitment to :ref:`a random number<random_seed>`, used to
    generate entropy on the chain. Present in only one out of
    ``BLOCKS_PER_COMMITMENT`` (see :ref:`Constants<ps_constants>`).
...
blocks<ps_constants>`, a cycle lasts *at least* 2 days, 20 hours, and 16
