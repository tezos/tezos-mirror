...
complementary to the existing :ref:`delegate <def_delegate_quebeca>`
...
subject to :ref:`slashing <slashing_quebeca>` if
the delegate misbehaves by :ref:`double-signing <def_double_signing_quebeca>`
...
:ref:`Participation rewards <adaptive_rewards_quebeca>` are automatically shared
...
policy <staking_policy_configuration_quebeca>` by setting staking parameters
...
4 cycles (cf. :ref:`Staked funds management <staked_funds_management_quebeca>`).
...
the same name introduced for :ref:`user accounts <def_user_account_quebeca>`.
...
.. _staking_policy_configuration_quebeca:
...
.. _staked_funds_management_quebeca:
