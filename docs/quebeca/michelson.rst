...
.. _transaction_semantics_quebeca:
...
Note that :ref:`manager operations batches <manager_operations_batches_quebeca>` contain a sequence of external operations signed as a whole by a source user account, which are executed atomically.
...
the :ref:`concrete syntax <ConcreteSyntax_quebeca>`. In particular
...
.. _michelson_type_system_quebeca:
...
However, one side effect of this RPC is the stripping of :ref:`annotations <annotations_quebeca>`.
...
Note that all annotations are stripped during :ref:`type normalization <type_normalization_quebeca>`.
...
:ref:`special annotations <SpecialAnnotations_quebeca>` and are not allowed everywhere.
...
The multisig commands of :ref:`Octez command line client <client_manual_quebeca>`
use this
...
:ref:`similar to Michelson scripts <syntax_of_scripts_quebeca>` but
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_quebeca>`).
...
:ref:`syntax of errors <syntax_of_errors_quebeca>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_quebeca>` can
...
- ``other_contracts`` (optional, defaults to ``{}``): mapping between
  ...
  specifications <syntax_of_other_contracts_quebeca>`)
...
- ``big_maps`` (optional, defaults to ``{}``): mapping between
  ...
  <syntax_of_extra_big_maps_quebeca>`)
...
.. _syntax_of_concrete_stacks_quebeca:
...
.. _omitting_parts_of_the_output_quebeca:
...
<syntax_of_concrete_operations_quebeca>`) or implementation-specific
parts of error outputs (see the :ref:`syntax of errors
<syntax_of_errors_quebeca>`).

.. _output_normalization_quebeca:
...
:ref:`wildcard pattern <omitting_parts_of_the_output_quebeca>` in the
...
- ``(StaticError <error description>)``: an error occurred before the
  ...
  <omitting_parts_of_the_output_quebeca>` such as ``(StaticError _)``
...
.. _syntax_of_concrete_operations_quebeca:
...
<omitting_parts_of_the_output_quebeca>`.
...
.. _syntax_of_other_contracts_quebeca:
...
.. _syntax_of_extra_big_maps_quebeca:
