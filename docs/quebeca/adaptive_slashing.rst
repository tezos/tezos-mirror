...
function<adaptive_slashing_fn_quebeca>` implementing this idea, as well as the
:ref:`new forbidden period<new_forbidden_period_quebeca>`.

.. _adaptive_slashing_fn_quebeca:

Adaptive Slashing Function
==========================

.. _adaptive_slashing_informal_quebeca:
...
.. _formal_adaptive_slashing_quebeca:
...
* :math:`\mathcal{W}` denotes the maximal possible *weight* of
  ...
  :ref:`slots<rights_quebeca>` in any block. It is also known as
  :ref:`CONSENSUS_COMMITTEE_SIZE<tb_validator_quebeca>`.
...
* :math:`T` is the *threshold* for the fraction of double attestations
  ...
  :ref:`CONSENSUS_THRESHOLD<tb_validator_quebeca>` which is set to
...
* the current total frozen stake for the delegate (sum of the
  :ref:`staking balances<active_stake_quebeca>` of the delegate itself
  and its stakers) is at least as high as the :ref:`active
  stake<active_stake_quebeca>` that was used ``CONSENSUS_RIGHTS_DELAY``
