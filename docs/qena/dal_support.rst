...
The DAL is integrated with :doc:`smart rollups <../active/smart_rollups>` so that kernels can request pages from the DAL via the :ref:`reveal data channel <reveal_data_channel_smart_rollups_qena>`. A smart rollup can fetch any page from the DAL node if the commitment respects some conditions:
...
This section describes the protocol constants specific to the DAL as well as their default values on mainnet (see :ref:`protocol_constants_qena` on how to find the values for tests networks):
