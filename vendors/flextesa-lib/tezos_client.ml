  ...
  let submit_proposals ?chain state keyed proposals =
    ...
    | Some _ ->
        ...
        say state
          EF.(
            desc
              (af "%s successfully submitted proposals: %s" keyed.client.id
                 (String.concat proposals))
              (ocaml_string_list res#out))

  let submit_ballot ?chain state keyed proposal ballot =
    match keyed.baker_hash with
    | None ->
    ...
    | Some _ ->
        ...
        successful_client_cmd state ~client:keyed.client
          ...
          @ [ "from"; "baker"; "contract"; keyed.baker_name; "submit"; "ballot"
            ; "for"; "protocol"; proposal; ballot ] )
        >>= fun res ->
        say state
          EF.(
            desc
              (af "%s successfully submitted ballot: %s - %s" keyed.client.id
                 ballot proposal)
              (ocaml_string_list res#out))
