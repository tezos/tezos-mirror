...
let protocol_parameters_json t : Ezjsonm.t =
  ...
  let extra_post_babylon_stuff subkind =
    ...
    let op_gas_limit, block_gas_limit =
      ...
      | `Delphi | `Edo | `Florence | `Florence_BA | `Alpha -> (1_040_000, 10_400_000) in
    let open Ezjsonm in
    ...
    ; ( match subkind with
      ...
      | `Carthage | `Delphi | `Edo | `Florence | `Florence_BA | `Alpha ->
    ...
    ; ( "cost_per_byte"
      , match subkind with
        ...
        | `Delphi | `Edo | `Florence | `Florence_BA | `Alpha -> string (Int.to_string 250) )
  ...
  | None ->
      dict
        ( match t.kind with
        | (`Babylon | `Carthage | `Delphi | `Edo | `Florence) as sk ->
            common @ extra_post_babylon_stuff sk
        | `Florence_BA | `Alpha as sk -> common @ extra_post_babylon_stuff sk @ extra_alpha
        | `Athens -> common )
