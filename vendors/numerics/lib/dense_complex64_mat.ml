...
type elt = Complex.t
type t = (Complex.t, Bigarray.complex64_elt, Bigarray.c_layout) Array2.t

let create_ lines cols =
  Array2.create Bigarray.Complex64 Bigarray.C_layout lines cols

let create ~(lines : int) ~(cols : int) =
  let mat = Array2.create Bigarray.Complex64 Bigarray.C_layout lines cols in
  Array2.fill mat Complex.zero ;
  mat
...
let map (mat : t) (f : Complex.t -> Complex.t) =
...
let add mat1 mat2 =
  ...
  Stubs.complex64_mat_add ~dst ~src1:mat1 ~src2:mat2 ;
  dst

let mul mat1 mat2 =
  let lines1, cols1 = shape mat1 in
  let dst = create_ lines1 cols1 in
  Stubs.complex64_mat_mul ~dst ~src1:mat1 ~src2:mat2 ;
  dst
