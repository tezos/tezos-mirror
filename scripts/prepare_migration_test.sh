...
$ ./scripts/prepare_migration_test.sh manual \\
...
$ ./scripts/prepare_migration_test.sh auto \\
  [<name_protocol>_<version_protocol>] </path/to/snapshot.rolling> [<block_hash>]

If the optional parameter <protocol_name>_<protocol_version> is provided, then
the script snapshots the Alpha protocol into a new folder
src/proto_<version_protocol>_<short_hash> and computes a realistic hash of the
protocol. Otherwise the script prepares the migration for the Alpha protocol in
the folder src/proto_alpha. It also patches the code to produce a yes-node.

The script imports a context from the snapshot file </path/to/snapshot.roll> and
places it in a folder with the same name than the snapshot file (without the
extension) under the system's temp directory. If the optional parameter
<block_hash> is provided, then the script checks that the hash of the last block
in the imported chain is <block_hash>.
"
...
then
    ...
    then
        ...
        tezt_env="TEZT_MIGRATION_TEST_PROTOCOL=$full_hash "
    fi

    import_snapshot "$snapshot_path" "$blockhash"
    [ -f "$context_dir/identity.json" ] && rm "$context_dir/identity.json"
    tezt_env="${tezt_env}TEZT_MIGRATION_TEST_CONTEXT=$context_dir "
...
$ ${tezt_env}dune exec ./tezt/manual_tests/main.exe -- --keep-temp migration

After the test ends, note down the path of the temporary files
$tmp_dir/tezt-XXXXXX, where XXXXXX are six random decimal figures.
