...
<RELEASE>: The release of the Linux distribution, e.g. 'jammy', 'focal', 'trixie'.
...
# /lib/systemd/system as 'aliased-location' on usr-merged systems. Whether a
# package ships its units under /lib or /usr/lib depends on the debhelper
# version it was built with (we pin no debhelper-compat), so our Debian
# packages can trip this. The proper fix is to ship the units under
# /usr/lib/systemd/system; until the packaging is updated we suppress the tag
