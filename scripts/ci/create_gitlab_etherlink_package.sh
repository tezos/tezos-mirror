...
gitlab_etherlink_debian_bookworm_package_url="${CI_API_V4_URL}/projects/${CI_PROJECT_ID}/packages/generic/${gitlab_etherlink_debian_bookworm_package_name}/${gitlab_package_version}"

gitlab_etherlink_ubuntu_focal_package_url="${CI_API_V4_URL}/projects/${CI_PROJECT_ID}/packages/generic/${gitlab_etherlink_ubuntu_focal_package_name}/${gitlab_package_version}"
gitlab_etherlink_ubuntu_jammy_package_url="${CI_API_V4_URL}/projects/${CI_PROJECT_ID}/packages/generic/${gitlab_etherlink_ubuntu_jammy_package_name}/${gitlab_package_version}"
...
gitlab_etherlink_source_package_url="${CI_API_V4_URL}/projects/${CI_PROJECT_ID}/packages/generic/${gitlab_etherlink_source_package_name}/${gitlab_package_version}"
...
for package in ${debian_bookworm_packages}; do
  ...
  gitlab_upload "./${package}" "${package_name}" "${gitlab_etherlink_debian_bookworm_package_url}"
done

echo "Upload Ubuntu focal packages"
for package in ${ubuntu_focal_packages}; do
  package_name="$(basename "${package}")"
  gitlab_upload "./${package}" "${package_name}" "${gitlab_etherlink_ubuntu_focal_package_url}"
done

echo "Upload Ubuntu jammy packages"
for package in ${ubuntu_jammy_packages}; do
  package_name="$(basename "${package}")"
  gitlab_upload "./${package}" "${package_name}" "${gitlab_etherlink_ubuntu_jammy_package_url}"
done
