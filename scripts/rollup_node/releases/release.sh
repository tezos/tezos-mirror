...
gitlab_release=$(echo "${CI_COMMIT_TAG}" | grep -oE '^(octez-smart-rollup-node|octez)-v[0-9]+(\.[0-9]+)?(-(rc|beta)([0-9]+))?$' || :)

# Strips the leading 'octez-smart-rollup-node-v'
# X.Y, X.Y-rcZ or X.Y-betaZ
gitlab_release_no_v=$(echo "${gitlab_release}" | sed -E 's/^(octez-smart-rollup-node|octez)-v//g')
