...
ReleaseCandidate | TestReleaseCandidate | Release | TestRelease | Beta | TestBeta)
  ...
  EXPECTED_VERSION="Octez $(echo "$gitlab_release_no_v" | tr '-' '~') build_number:0"
  ;;
Rebuild | TestRebuild)
  DEBVERSION=$VERSION
  DEBCHANGELOG="New Release $VERSION / $CI_COMMIT_SHORT_SHA"
  EXPECTED_VERSION="Octez ${gitlab_release_major_version}.${gitlab_release_minor_version} build_number:${gitlab_packaging_revision_version}"
  ;;
Master)
