...
asset_json="$(jq -c -n --arg name "$source (source)" \
                       --arg url "$PACKAGE_REGISTRY_URL/$source" \
                        '{name: $name, url: $url}')"
assets="$assets --assets-link='$asset_json'"

for binary in $binaries; do
    asset_json="$(jq -c -n --arg name "$binary (x86_64 Linux)" \
                           --arg url "$PACKAGE_REGISTRY_URL/x86_64-$binary" \
                           '{name: $name, url: $url}')"
    assets="$assets --assets-link='$asset_json'"
    asset_json="$(jq -c -n --arg name "$binary (arm64 Linux)" \
                           --arg url "$PACKAGE_REGISTRY_URL/arm64-$binary" \
                           '{name: $name, url: $url}')"
    assets="$assets --assets-link='$asset_json'"
done

archive_json="$(jq -c -n --arg name "x86_64-linux-tezos-binaries.tar.gz" \
                         --arg url "$PACKAGE_REGISTRY_URL/x86_64-tezos-binaries.tar.gz" \
                         '{name: $name, url: $url}')"
assets="$assets --assets-link='$archive_json'"
archive_json="$(jq -c -n --arg name "arm64-linux-tezos-binaries.tar.gz" \
...
changelog_json="$(jq -c -n --arg url "https://tezos.gitlab.io/CHANGES.html#version-$CI_COMMIT_TAG_NORMALIZED" \
                      '{name: "Changelog", link_type: "other", url: $url}')"
assets="$assets --assets-link='$changelog_json'"

MAJOR_VERSION="$(echo "$CI_COMMIT_TAG" | sed -e "s/v//g" | cut -d. -f1)"
announcement_json="$(jq -c -n --arg url "https://tezos.gitlab.io/releases/version-$MAJOR_VERSION.html" \
                      '{name: "Announcement", link_type: "other", url: $url}')"
assets="$assets --assets-link='$announcement_json'"
...
docker_images_json="$(jq -c -n --arg url "https://hub.docker.com/layers/tezos/tezos/v$CI_COMMIT_TAG_WITHOUT_TILDE/images/sha256-$IMAGE_DIGEST" \
...
eval "$(echo release-cli create --name \"Release "$CI_COMMIT_TAG"\" --tag-name \""$CI_COMMIT_TAG"\" "$assets")"
