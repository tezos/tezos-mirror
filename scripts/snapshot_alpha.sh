...
# rename .opam files
for file in $(find opam/ -name \*alpha\*.opam)
do
    new_opam_file=$(echo "$file" | sed s/alpha/${version}-${short_hash}/g)
    cp "$file" $new_opam_file
...
# Patch dune-project file
cp dune-project dune-project.old
grep alpha dune-project.old | sed "s/-alpha/-${version}-${short_hash}/g"  >> dune-project

(cd src/proto_${version}_${short_hash}/lib_protocol/ && \
     dune exec \
              ../../lib_protocol_compiler/bin/replace.exe \
              ../../lib_protocol_compiler/dune_protocol.template.v1 \
              dune.inc \
              ../../lib_protocol_compiler/final_protocol_versions ${version}_${short_hash})
