...
if [ -n "$dev" ]; then
  ...
  opam install --yes opam/virtual/octez-deps.opam.locked opam/virtual/octez-dev-deps.opam opam/virual/release-tools-deps.opam --deps-only --criteria="-changed,-removed"
fi
