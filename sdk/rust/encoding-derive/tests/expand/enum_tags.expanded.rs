...
impl tezos_data_encoding::encoding::HasEncoding for EnumTags
where
...
impl<'_a> tezos_data_encoding::nom::NomReader<'_a> for EnumTags
where
    ...
    fn nom_read(bytes: &'_a [u8]) -> tezos_data_encoding::nom::NomResult<'_a, Self> {
        (|input| {
            ...
            let (input, variant) = if tag == 0 {
                ...
                (|bytes| Ok((bytes, EnumTags::B)))(input)?
            } else {
                return Err(
                    nom::Err::Error(
...
#[allow(clippy::unnecessary_cast)]
#[allow(clippy::redundant_closure_call)]
impl tezos_data_encoding::enc::BinWriter for EnumTags
where
