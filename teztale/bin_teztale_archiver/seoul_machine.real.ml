...
module Block_services =
  Tezos_client_seoul.Protocol_client_context.Alpha_block_services

open Lwt_result_syntax
open Tezos_protocol_seoul
open Tezos_protocol_plugin_seoul

module Services : Protocol_machinery.PROTOCOL_SERVICES = struct
  let hash = Protocol.hash

  type wrap_full = Tezos_client_seoul.Protocol_client_context.wrap_full

  let wrap_full cctxt =
    new Tezos_client_seoul.Protocol_client_context.wrap_full cctxt
            ...
            Consensus_ops.
              {
                ...
                delegate = Tezos_crypto.Signature.Of_V2.public_key_hash delegate;
