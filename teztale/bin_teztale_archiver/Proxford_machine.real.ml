...
module Block_services =
  Tezos_client_018_Proxford.Protocol_client_context.Alpha_block_services
...
module Services : Protocol_machinery.PROTOCOL_SERVICES = struct
  ...
  type wrap_full = Tezos_client_018_Proxford.Protocol_client_context.wrap_full

  let wrap_full cctxt =
    new Tezos_client_018_Proxford.Protocol_client_context.wrap_full cctxt
    ...
    | {
     Protocol.Main.protocol_data =
       Protocol.Alpha_context.Operation_data
         {
           contents =
             Single (Attestation {slot; level; round; block_payload_hash});
           signature = _;
         };
     shell = _;
    } ->
    ...
    match
      List.rev_map
        (fun ({delegate; round; _} : RPC.Baking_rights.t) ->
          {Data.delegate; round = Protocol.Alpha_context.Round.to_int32 round})
        baking_rights
    with
