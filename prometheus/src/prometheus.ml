...
module MetricInfo = struct
  ...
  let pp_opt () = function None -> "" | Some v -> v ^ "_"
...
module CollectorRegistry = struct
  type t = {
    ...
    mutable metrics_lwt :
      (unit -> Sample_set.t LabelSetMap.t Lwt.t) MetricFamilyMap.t;
  ...
  let ensure_not_registered t info =
    if
      MetricFamilyMap.mem info t.metrics
  ...
  let collect t =
    ...
    MetricFamilyMap.merge
      (fun _ v1 v2 -> match v1 with Some v1 -> Some v1 | None -> v2)
      metrics
      metrics_lwt
end
...
module Metric (Child : CHILD) : sig
  ...
  let v_labels ~label_names ?(registry = CollectorRegistry.default) ~help
  ...
  let labels t label_values =
    assert (
      List.length t.metric.MetricInfo.label_names = List.length label_values) ;
...
module Summary = struct
  module Child = struct
    type t = {mutable count : float; mutable sum : float}
