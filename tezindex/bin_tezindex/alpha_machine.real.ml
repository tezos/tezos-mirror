...
module Services : Protocol_machinery.PROTOCOL_SERVICES = struct
  ...
  let get_balance_update
    ...
    | CLST_deposits, _, Block_application
    | CLST_redeemed_deposits _, _, Block_application ->
        None
  ...
  let destination_of_update = function
    | Contract (Implicit address) -> Some (address, Data.Balance_update.Contract)
    | Contract (Originated _) -> None
    | Baker delegate -> Some (delegate, Data.Balance_update.Baker_own_stake)
    | Staker (Implicit pkh, _delegate) -> Some (pkh, Data.Balance_update.Staker)
    | Staker (Originated _, _) -> None
    | Shared delegate -> Some (delegate, Data.Balance_update.Delegate)
    | Edge delegate -> Some (delegate, Data.Balance_update.Baker_edge)
    | Lost_attestation addr -> Some (addr, Data.Balance_update.Lost)
    | Lost_dal_attestation addr -> Some (addr, Data.Balance_update.Lost)
