...
let setup_pool ?name num_domains = T.setup_pool ?name ~num_domains ()
...
let detach pool f args =
  if get_num_domains pool = 1 then Lwt.wrap1 f args
  else
    let result = ref init_result in
    let task () = result := try Ok (f args) with exn -> Error exn in
...
let run_in_main f =
  ...
  C.recv job_done;
  match !res with Result.Ok ret -> ret | Result.Error exn -> raise exn
