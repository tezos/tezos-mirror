...
Validation logic:
  ...
  2. For every autogenerated parser from 1., we test that it succeeds parsing
     ...
     The binary blobs are generated using 'xxd' program, from the corresponding
     '.hex' input files, that are pre-generated using 'generate_input_samples.sh'
     and stored inside HEX_INPUT_DIR.

     For every [encoding].ksy file inside KSY_DIR, $(basename "$0") expects the
     following entries:
      - HEX_INPUT_DIR/valid/[encoding]/*.hex (valid inputs).
      - HEX_INPUT_DIR/invalid/[encoding]/*.hex (invalid inputs).

Logging:
  For a given '.ksy' file, given the result of validation, we log to standard
  error:
    1. The success status in case of successfull validation.
    2. All context, e.g. auto generated parser, hex input we are testing on,
       error trace, etc. in case of a failing validation.
    3. Warning in case of missing valid input files.
    4. Warning in case of missing invalid input files.

Validation results:
  Following validation of all '.ksy' files, we log the aggregated results
  to standard output by grouping:
    1. Files with no corresponding valid hex input files. Those files we don't
       even test and count validation as failed.
    2. Files with missing invalid hex input files. For now missing invalid input
       files do not affect validation status and only result in emitted warning.
    3. Files that passed validation, i.e. they parse valid or fail parsing
       invalid input samples.
    4. Files that failed validation, i.e. they parse invalid or fail parsing
       valid input samples.

  In case of no failed validation, i.e. the sum of valid and untested files is
  equal to the sum of all '.ksy' files inside KSY_DIR, we exit with 0 exit code.
  Else we exit with 1.
...
- generate_input_samples.sh that should be located in the same directory as
  $(basename "$0"). This script generates input hex samples using octez-codec
  to dump the data.
EOF
  exit 1
}
...
KSY_DIR="$1"
# Directory with corresponding test inputs for tested .ksy files.
HEX_INPUT_DIR="$tmp/input"
mkdir "$HEX_INPUT_DIR"
generation_output=$(./"$DIR"/generate_input_samples.sh "$HEX_INPUT_DIR" 2>&1)
# It is critical that we don't silently propagate any errors in hex input
# samples generation, as this might give us false positive validation (!).
generation_status=$?
if [ $generation_status -eq 0 ]; then
  log "Input files generation successful."
else
  echo "generate_input_samples.sh throws an error:"
