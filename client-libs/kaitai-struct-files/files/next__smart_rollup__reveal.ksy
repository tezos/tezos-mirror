meta:
  id: next__smart_rollup__reveal
  endian: be
doc: ! 'Encoding id: next.smart_rollup.reveal'
types:
  input_hash:
    seq:
    - id: input_hash_tag
...
enums:
  ...
  next__contract_id_tag:
    0: implicit
    1: originated
  next__smart_rollup__reveal_tag:
    0: reveal_raw_data
    1: reveal_metadata
    2: request_dal_page
  ...
  restricted_commitments_publishers_tag:
    ...
    1: some
seq:
- id: next__smart_rollup__reveal_tag
  type: u1
  enum: next__smart_rollup__reveal_tag
- id: reveal_raw_data
  type: input_hash
  if: (next__smart_rollup__reveal_tag == next__smart_rollup__reveal_tag::reveal_raw_data)
- id: request_dal_page
  type: page_id
  if: (next__smart_rollup__reveal_tag == next__smart_rollup__reveal_tag::request_dal_page)
- id: request_adaptive_dal_page
  type: request_adaptive_dal_page
  if: (next__smart_rollup__reveal_tag == next__smart_rollup__reveal_tag::request_adaptive_dal_page)
