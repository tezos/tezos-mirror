...
`runtime_datadog` and additionally copies some binaries from
`build`:

```mermaid
graph TB
  runtime -->|FROM| runtime_datadog
  runtime_datadog -->|FROM| prebuild
  runtime_datadog -->|FROM| e2etest
  prebuild -->|FROM| build
  build -->|FROM| test
...
| `runtime_datadog` | datadog-ci (via npm)               | base image to instrument CI jobs  |
