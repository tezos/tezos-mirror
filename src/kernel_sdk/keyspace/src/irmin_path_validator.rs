...
use tezos_smart_rollup_host::path::{
    validate_path, validate_path_internal, PATH_MAX_SIZE, PATH_SEPARATOR,
};
...
impl Key {
    ...
    pub(super) const fn check_bytes(bytes: &[u8]) -> Result<(), KeyError> {
        ...
        match validate_path_internal(bytes) {
            Ok(()) => Ok(()),
...
impl Name {
    ...
    /// Must not be `/readonly` (reserved by the rollup node).
...
mod tests {
    ...
    fn overlaps_identical() {
        let a = Name::from_slice("/evm").unwrap();
        let b = Name::from_slice("/evm").unwrap();
        assert!(a.overlaps(b));
    }
    ...
    fn overlaps_prefix_shorter_first() {
        let a = Name::from_slice("/evm").unwrap();
        let b = Name::from_slice("/evm/world_state").unwrap();
        assert!(a.overlaps(b));
    }

    #[test]
    fn overlaps_prefix_longer_first() {
        let a = Name::from_slice("/evm/world_state").unwrap();
        let b = Name::from_slice("/evm").unwrap();
        assert!(a.overlaps(b));
    }
    ...
    fn no_overlap_different_paths() {
        let a = Name::from_slice("/evm/world").unwrap();
        let b = Name::from_slice("/evm/world_state").unwrap();
        assert!(!a.overlaps(b));
    }
    ...
    fn no_overlap_partial_step_name() {
        ...
        let a = Name::from_slice("/evm").unwrap();
        let b = Name::from_slice("/evmstuff").unwrap();
        assert!(!a.overlaps(b));
    }

    #[test]
    fn overlaps_deep_nesting() {
        let a = Name::from_slice("/a/b").unwrap();
        let b = Name::from_slice("/a/b/c/d").unwrap();
        assert!(a.overlaps(b));
        assert!(b.overlaps(a));
    }
    ...
    fn readonly_prefix_allowed() {
        ...
        assert!(Name::from_slice("/readonly_stuff").is_ok());
    }
}
