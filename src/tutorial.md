...
You can use the `Json_only` tag to indicate that a case is only to be tried in
the JSON backend. This can be useful to create different representations of the
same type for the different backends, to delegate JSON
serialisation/deserialisation to a separate routine, or to interfere in some other
ways with the deserialisation process.
...
- `` `N `` represents it on a variable number of bytes (as per the `n` encoding)
  and allows sizes up to 2^30 bytes.
...
There are two common approach to ensure this.
