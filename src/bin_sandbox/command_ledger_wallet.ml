...
let with_ledger_test_reject_and_accept ?(only_success = false) state ~messages f
  ...
  let with_ledger_prompt state ~messages ~user_answer ~f =
    ledger_prompt_notice_expectation state ~messages ~user_answer >>= fun () ->
    f ~user_answer
  in
...
let voting_tests state ~client ~src ~with_rejections ~protocol_kind
  ...
  let test_reject_and_accept name ~messages action =
    (if with_rejections then
     ...
     >>= fun () ->
     action () >>= fun res -> expect_rejection name res
    else return ())
    >>= fun () ->
    ledger_prompt_notice_expectation state ~messages ~user_answer:`Accept
    >>= fun () ->
    action () >>= fun res -> expect_failure name res
  in
            ...
            | `Hangzhou | `Ithaca | `Alpha ->
                wf
                  ...
                  "From Babylon on, You will first be asked to provide the \
                   public key." ;
                cut ppf () ;
                wf ppf "Accept this prompt, regardless of below, then continue.");
          (fun ppf () ->
            vertical_box ppf ~indent:4 (fun ppf ->
                wf
                  ppf
                  "Protocol is %a, the ledger should be able to display voting \
                   parameters:"
...
let originate_manager_tz_script state ~client ~name ~from ~bake ~protocol_kind
  ...
  let prepare_origination_of_manager_tz_script ?(spendable = false)
    ...
    System.write_file state tmp ~content:manager_tz_script >>= fun () ->
    let origination =
...
let manager_tz_delegation_tests state ~client ~ledger_key ~ledger_account
  ...
  in
  return () >>= fun () ->
  set_delegation () >>= fun _ ->
  ksprintf bake "setting delegate of %s" contract_address >>= fun () ->
  remove_delegation () >>= fun _ ->
  ksprintf bake "removing delegate of %s" contract_address >>= fun () ->
  manager_tz_to_implicit () >>= fun _ ->
  ksprintf bake "transferring from %s to %s" contract_address random_acc_pkh
  >>= fun () ->
  manager_tz_to_originated_large () >>= fun _ ->
  ksprintf bake "transfering from %s to %s" contract_address random_contract_pkh
  >>= fun () ->
  manager_tz_to_originated_small () >>= fun _ ->
  ksprintf bake "transfering from %s to %s" contract_address random_contract_pkh
...
let transaction_tests state ~client ~src ~with_rejections ~protocol_kind
  ...
  in
  test_transaction ~name:"Self-transaction" ~dst_pkh:ledger_pkh ~dst_name:src ()
  >>= fun () ->
  ...
  let test_origination ?delegate ?delegatable ?spendable ?push_drops ~name
    ...
    with_ledger_test_reject_and_accept
      ...
      ~messages:
        MFmt.
          [
            (fun ppf () -> wf ppf "Origination: %s (ledger: %s)" name ledger_pkh);
...
let run state ~pp_error ~protocol ~protocol_kind ~node_exec ~client_exec
  ...
  in
  Tezos_client.Keyed.initialize state baker_0 >>= fun _ ->
  ...
  bake "After transferring tez to the ledger account" >>= fun () ->
  with_ledger_test_reject_and_accept
      ...
      (fun sexps ->
        Asynchronous_result.transform_error
          ...
          | [Atom a] -> (
              ...
              | Some `All ->
                  run delegation_tests >>= fun () ->
                  run manager_tz_delegation_tests >>= fun () ->
                  run batch_test >>= fun () ->
                  run transactions_test >>= fun () ->
