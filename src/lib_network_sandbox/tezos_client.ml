...
let activate_protocol_script t ~state protocol =
  let open Genspio.EDSL in
  check_sequence
    ~verbosity:(`Announce "activating-protocol")
    [ ( "add-activator-key",
        import_secret_key_script
          t
          ~state
          ...
          ~how:
            [ ( "activate",
                ...
                @ [ "activate";
                    ...
                    "and";
                    "parameters";
                    Tezos_protocol.protocol_parameters_path
                      ~config:state
                      protocol ] ) ] ) ]

let import_secret_key t ~state name key =
  Running_processes.run_genspio
    state
  ...
  try
    ...
    say
      ...
      EF.(
        desc
          (af
             "looking for %S in block %d: %sfound"
             kind
             level
         ...
         Re.(
           fun line ->
             ...
             | None ->
                 None
             | Some matches ->
                 Some (Group.get matches 1, Group.get matches 2)))
...
module Ledger = struct
  ...
  let show_ledger state ~client ~uri =
    ...
    try
      ...
      in
      return
        (Tezos_protocol.Account.key_pair
           name
           ~pubkey
           ~pubkey_hash
           ~private_key:uri)
    with e ->
      failf
        "Couldn't understand result of 'show ledger %S': error %S: from %S"
        uri
        (Exn.to_string e)
        (String.concat ~sep:"\n" res#out)
    ...
    return
      Re.(
        ...
        | Some _ ->
            None
        | None ->
            Some (Group.get (exec re_uri out) 1))
end
...
module Keyed = struct
  ...
  let bake ?chain state baker msg =
    ...
    >>= fun res ->
    Log_recorder.Operations.bake
      state
      ~client:baker.client.id
      ~output:res#out
      msg ;
    say
      state
  ...
  let endorse state baker msg =
    ...
    >>= fun res ->
    Log_recorder.Operations.endorse
      state
      ~client:baker.client.id
      ~output:res#out
      msg ;
