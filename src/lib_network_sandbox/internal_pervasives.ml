...
module Process_result = struct
  ...
  let status_to_string s =
    Lwt_unix.(
      match s with
      | WEXITED n ->
          sprintf "exited with %d" n
      | WSIGNALED n ->
          sprintf "was signaled: %d" n
      | WSTOPPED n ->
          sprintf "was stopped: %d" n)
    ...
    let pp fmt = function
      | (`Wrong_status (res, msg) : [< t]) ->
          Format.(
            fprintf
              fmt
              "Process-error, wrong status:@ '%s':@ %s"
...
module Jqo = struct
  ...
  let field ~k = function
    | `O l ->
        List.Assoc.find_exn l ~equal:String.equal k
    | other ->
        ksprintf failwith "Jqo.field (%S) in %s" k (to_string other)
  ...
  let list_find ~f = function
    ...
    | `A l ->
        List.find_exn ~f l
    | other ->
        ksprintf failwith "Jqo.list_find in %s" (to_string other)
  ...
  let remove_field o ~name =
    match o with
    | `O l ->
        `O (List.filter l ~f:(fun (k, _) -> k <> name))
    | other ->
        ksprintf
          failwith
