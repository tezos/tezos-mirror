  ...
  let exn_tezos msg = function
    ...
    | Error el ->
        Format.kasprintf
          ...
          "Script-error: %s: %a"
          msg
          Tezos_error_monad.Error_monad.pp_print_error
          el

  let exn_shell msg res = Environment.wrap_error res |> exn_tezos msg

  let parse exprs =
    Michelson_v1_parser.((parse_expression exprs |> fst).expanded)
  ...
  let json_script_repr code storage =
    match
      Tezos_data_encoding.Data_encoding.Json.construct
        Script_repr.encoding
        Script_repr.{code = lazy_expr code; storage = lazy_expr storage}
    with
    | `O _ as o ->
        (o : Ezjsonm.t)
    | _other ->
...
let cli_term () =
  ...
  $ Arg.(
      value
        (flag
           (info
              ~doc:"Do not create any of the default bootstrap accounts."
              ~docs
