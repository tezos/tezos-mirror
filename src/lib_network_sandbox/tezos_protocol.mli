...
module Script : sig
  ...
  val parse : string -> Alpha_context.Script.expr

  val code_of_json_exn :
    string -> Michelson_v1_primitives.prim Tezos_micheline.Micheline.canonical

  val json_script_repr : Script_repr.expr -> Script_repr.expr -> Ezjsonm.t

  val original_json : string

  val faucet_tz : string

  val print : Script_repr.expr -> Script_repr.expr -> unit
