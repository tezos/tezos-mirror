...
data :
  | LPAR DATA bind_var_opt string_list RPAR
    { let at = at () in
      ...
      fun () -> {dinit = Chunked_byte_vector.Lwt.Buffer.of_string $4; dmode = Passive @@ at} @@ at }
  | LPAR DATA bind_var_opt memory_use offset string_list RPAR
    { let at = at () in
      fun c -> ignore ($3 c anon_data bind_data);
      fun () ->
      {dinit = Chunked_byte_vector.Lwt.Buffer.of_string $6; dmode = Active {index = $4 c memory; offset = $5 c} @@ at} @@ at }
  | LPAR DATA bind_var_opt offset string_list RPAR  /* Sugar */
    { let at = at () in
      fun c -> ignore ($3 c anon_data bind_data);
      fun () ->
      {dinit = Chunked_byte_vector.Lwt.Buffer.of_string $5; dmode = Active {index = 0l @@ at; offset = $4 c} @@ at} @@ at }
...
memory_fields :
  ...
  | LPAR DATA string_list RPAR  /* Sugar */
    { fun c x at ->
      ...
      [{dinit = Chunked_byte_vector.Lwt.Buffer.of_string $3; dmode = Active {index = x; offset} @@ at} @@ at],
      [], [] }
