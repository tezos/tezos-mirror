...
func_type :
  ...
  | LPAR PARAM value_type_vector RPAR func_type
    { let FuncType (ins, out) = $5 in FuncType (Vector.unsafe_concat $3 ins, out) }
...
var :
  | NAT { let at = at () in fun _ _lookup -> nat32 $1 at @@ at }
...
plain_instr :
  ...
  | TABLE_GET { let at = at () in fun _ -> table_get (0l @@ at) }  /* Sugar */
  | TABLE_SET { let at = at () in fun _ -> table_set (0l @@ at) }  /* Sugar */
  | TABLE_SIZE { let at = at () in fun _ -> table_size (0l @@ at) }  /* Sugar */
  | TABLE_GROW { let at = at () in fun _ -> table_grow (0l @@ at) }  /* Sugar */
  | TABLE_FILL { let at = at () in fun _ -> table_fill (0l @@ at) }  /* Sugar */
  | TABLE_COPY  /* Sugar */
    { let at = at () in fun _ -> table_copy (0l @@ at) (0l @@ at) }
  | TABLE_INIT var  /* Sugar */
    { let at = at () in fun c -> table_init (0l @@ at) ($2 c elem) }
  | ELEM_DROP var { fun c -> elem_drop ($2 c elem) }
  | LOAD offset_opt align_opt { fun _ -> $1 $3 $2 }
  | STORE offset_opt align_opt { fun _ -> $1 $3 $2 }
  | VEC_LOAD offset_opt align_opt { fun _ -> $1 $3 $2 }
  | VEC_STORE offset_opt align_opt { fun _ -> $1 $3 $2 }
  | VEC_LOAD_LANE offset_opt align_opt NAT
    { let at = at () in fun _ -> $1 $3 $2 (vec_lane_index $4 at) }
  | VEC_STORE_LANE offset_opt align_opt NAT
    { let at = at () in fun _ -> $1 $3 $2 (vec_lane_index $4 at) }
  | MEMORY_SIZE { fun _ -> memory_size }
  | MEMORY_GROW { fun _ -> memory_grow }
  | MEMORY_FILL { fun _ -> memory_fill }
  | MEMORY_COPY { fun _ -> memory_copy }
  | MEMORY_INIT var { fun c -> memory_init ($2 c data) }
  | DATA_DROP var { fun c -> data_drop ($2 c data) }
  | REF_NULL ref_kind { fun _ -> ref_null $2 }
  | REF_IS_NULL { fun _ -> ref_is_null }
  | REF_FUNC var { fun c -> ref_func ($2 c func) }
  | CONST num { fun _ -> fst (num $1 $2) }
  | TEST { fun _ -> $1 }
  | COMPARE { fun _ -> $1 }
  | UNARY { fun _ -> $1 }
  | BINARY { fun _ -> $1 }
  | CONVERT { fun _ -> $1 }
  | VEC_CONST VEC_SHAPE num_list { let at = at () in fun _ -> fst (vec $1 $2 $3 at) }
  | VEC_UNARY { fun _ -> $1 }
  | VEC_BINARY { fun _ -> $1 }
  | VEC_TERNARY { fun _ -> $1 }
  | VEC_TEST { fun _ -> $1 }
  | VEC_SHIFT { fun _ -> $1 }
  | VEC_BITMASK { fun _ -> $1 }
  | VEC_SHUFFLE num_list { let at = at () in fun _ -> i8x16_shuffle (shuffle_lit $2 at) }
  | VEC_SPLAT { fun _ -> $1 }
  | VEC_EXTRACT NAT { let at = at () in fun _ -> $1 (vec_lane_index $2 at) }
  | VEC_REPLACE NAT { let at = at () in fun _ -> $1 (vec_lane_index $2 at) }


select_instr :
  | SELECT select_instr_results
    { let at = at () in fun _ -> let b, ts = $2 in
...
call_instr_params_instr :
  | LPAR PARAM value_type_vector RPAR call_instr_params_instr
    { fun c ->
      let FuncType (ts1, ts2), es = $5 c in FuncType (Vector.unsafe_concat $3 ts1, ts2), es }
...
call_instr_results_instr :
  | LPAR RESULT value_type_vector RPAR call_instr_results_instr
    { fun c -> let ts, es = $5 c in Vector.unsafe_concat $3 ts, es }
  | instr
...
call_expr_params :
  | LPAR PARAM value_type_vector RPAR call_expr_params
    { fun c ->
      ...
      FuncType (Vector.unsafe_concat $3 ts1, ts2), es }
  | call_expr_results
    { fun c -> let ts, es = $1 c in FuncType (Vector.empty (), ts), es }
...
func_fields_import_result :  /* Sugar */
  ...
  | LPAR RESULT value_type_vector RPAR func_fields_import_result
    { let FuncType (ins, out) = $5 in FuncType (ins, Vector.unsafe_concat $3 out) }

func_fields_body :
  | func_result_body { $1 }
  | LPAR PARAM value_type_vector RPAR func_fields_body
    { let FuncType (ins, out) = fst $5 in
      FuncType (Vector.unsafe_concat $3 ins, out),
...
func_body :
  ...
  | LPAR LOCAL value_type_vector RPAR func_body
    { fun c -> anon_locals_vector c (lazy $3); let f = $5 c in
      {f with locals = Vector.unsafe_concat $3 f.locals} }
