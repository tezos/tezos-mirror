...
struct
  ...
  let rec instr e =
    ...
    | Load {ty = I32Type | I64Type; pack = Some (Pack64, _); _} -> assert false
    ...
    | VecLoad ({ty = V128Type; pack = None; _} as mo) ->
        ...
        memop mo
    | VecLoad
        ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack8x8, SX)); _} as mo)
      ->
        vecop 0x01l ;
        memop mo
    | VecLoad
        ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack8x8, ZX)); _} as mo)
      ->
        vecop 0x02l ;
        memop mo
    | VecLoad
        ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack16x4, SX)); _} as mo)
      ->
        vecop 0x03l ;
        memop mo
    | VecLoad
        ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack16x4, ZX)); _} as mo)
      ->
        vecop 0x04l ;
        memop mo
    | VecLoad
        ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack32x2, SX)); _} as mo)
      ->
        vecop 0x05l ;
        memop mo
    | VecLoad
        ({ty = V128Type; pack = Some (Pack64, ExtLane (Pack32x2, ZX)); _} as mo)
      ->
        vecop 0x06l ;
        memop mo
  ...
  let code f =
    ...
    let locals =
      List.filter_map snd (Lazy_vector.Int32Vector.loaded_bindings locals)
    in
    let g = gap32 () in
    let p = pos s in
  ...
  let elem seg =
    ...
    let einit =
      List.filter_map snd (Lazy_vector.Int32Vector.loaded_bindings einit)
    in
