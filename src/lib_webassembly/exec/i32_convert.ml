...
let trunc_f32_s x =
  if F32.ne x x then raise Ixx.InvalidConversion
  else
...
let trunc_f64_s x =
  if F64.ne x x then raise Ixx.InvalidConversion
  else
    let xf = F64.to_float x in
    if xf >= -.Int32.(to_float min_int) || xf <= Int32.(to_float min_int) -. 1.0
    then raise Ixx.Overflow
    else Int32.of_float xf
