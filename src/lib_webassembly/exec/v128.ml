...
module MakeIntShape (IXX : Ixx.S) (Cvt : Convert(IXX).S) :
  IntShape with type lane = IXX.t = struct
  type lane = IXX.t
  ...
  let reduceop f a s =
    List.fold_left (fun a b -> f a (b <> IXX.zero)) a (to_lanes s)
  ...
  let bitmask x =
    ...
    let negs =
      List.map (fun x -> if IXX.(lt_s x zero) then Int32.one else Int32.zero) xs
    in
    List.fold_right
      (fun a b -> Int32.(logor a (shift_left b 1)))
      negs
      Int32.zero
...
module MakeFloatShape (FXX : Fxx.S) (Cvt : Convert(FXX).S) :
  FloatShape with type lane = FXX.t = struct
  type lane = FXX.t
...
module I8x16 =
  MakeIntShape
    ...
    (struct
      ...
      let to_lanes s =
        List.init 16 (fun i ->
            Int32.of_int (Bytes.get_int8 (Bytes.of_string s) i))
...
module I16x8 =
  MakeIntShape
    ...
    (struct
      ...
      let to_lanes s =
        List.init 8 (fun i ->
            Int32.of_int (Bytes.get_int16_le (Bytes.of_string s) (i * 2)))
...
module I32x4 =
  MakeIntShape
    ...
    (struct
      ...
      let to_lanes s =
        List.init 4 (fun i ->
            I32.of_bits (Bytes.get_int32_le (Bytes.of_string s) (i * 4)))
...
module I64x2 =
  MakeIntShape
    ...
    (struct
      ...
      let to_lanes s =
        List.init 2 (fun i ->
            I64.of_bits (Bytes.get_int64_le (Bytes.of_string s) (i * 8)))
...
module F32x4 =
  MakeFloatShape
    ...
    (struct
      ...
      let to_lanes s =
        List.init 4 (fun i ->
            F32.of_bits (Bytes.get_int32_le (Bytes.of_string s) (i * 4)))
...
module F64x2 =
  MakeFloatShape
    ...
    (struct
      ...
      let to_lanes s =
        List.init 2 (fun i ->
            F64.of_bits (Bytes.get_int64_le (Bytes.of_string s) (i * 8)))
...
module V8x16 = struct
  let swizzle v1 v2 =
    ...
    in
    I8x16.of_lanes (List.map select is)
...
let of_strings shape ss =
  ...
  | I16x8 () ->
      List.iteri
        (fun i s -> set_int16_le b (i * 2) (I16.to_int_u (I16.of_string s)))
        ss
  | I32x4 () ->
  ...
  | F32x4 () ->
      List.iteri
        (fun i s -> set_int32_le b (i * 4) (F32.to_bits (F32.of_string s)))
        ss
  | F64x2 () ->
      List.iteri
