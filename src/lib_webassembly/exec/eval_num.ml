...
module IntOp (IXX : Ixx.S) (Num : NumType with type t = IXX.t) = struct
  open Ast.IntOp
  open Num
  ...
  let relop op =
    let f =
      ...
      | GeU -> IXX.ge_u
    in
    fun v1 v2 -> f (of_num 1 v1) (of_num 2 v2)
end
...
module FloatOp (FXX : Fxx.S) (Num : NumType with type t = FXX.t) = struct
  open Ast.FloatOp
  open Num
