...
let vectors =
  [
    ...
    };
    {
      ...
      words =
        "light rule cinnamon wrap drastic word pride squirrel upgrade then \
    ...
    };
    {
      ...
      words =
        "void come effort suffer camp survey warrior heavy shoot primary \
         clutch crush open amazing screen patrol group space point ten exist \
         slush involve unfold";
      seed =
        `Hex
  ...
  | _ ->
      ...
      @@ ListLabels.fold_left2 l1 l2 ~init:0 ~f:(fun i w1 w2 ->
             if w1 <> w2 then
               Format.fprintf
                 ppf
                 "At position %d, words differ: %s <> %s"
                 i
                 w1
                 w2 ;
             succ i)
