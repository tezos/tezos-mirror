...
module type REGISTRY = sig
  ...
  (** [hash registry] computes the Merkle root hash of [registry],
      derived from the root hashes of all its databases.  Normal and
      Prove return [bytes] directly from the Rust binding; Verify
      converts a [verification_error] return into a raised
      {!Nds_errors.Verification_failed}, so the signature stays
      [bytes] across modes.  Catch the exception at the
      {!Nds.with_verification} boundary rather than at individual
      call sites. *)
  val hash : t -> bytes

  (** [size registry] returns the number of databases currently held in
      [registry].  Same exception convention as {!hash}: Normal/Prove
      return [int64] directly, Verify raises
      {!Nds_errors.Verification_failed} on a proof mismatch. *)
  val size : t -> int64
...
(** Verify mode: registry + database + verify entry point.
    ...
    they produce the expected state transition.  Proof-mismatch failures
    are signalled out-of-band by raising {!Nds_errors.Verification_failed},
    so a Verify backend can be passed directly to [Nds.wrap] alongside
    Normal backends without an adapter.  Consumers catch the exception at
    a known boundary (typically the kernel-step replay's [Lwt.catch]
    frame) to reject a malformed proof. *)
module type VERIFY = sig
  ...
  (** [start_verify proof] creates a verify-mode registry that replays
      ...
      transition.  Mismatches raise {!Nds_errors.Verification_failed}
      from the operation that first hits the divergence. *)
