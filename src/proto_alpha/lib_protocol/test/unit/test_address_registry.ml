...
(* This function could be replaced by an `Internal_for_test.set_counter`
   function. *)
let init_counter ctxt i =
  let open Lwt_result_wrap_syntax in
  let rec loop (ctxt, curr_counter) =
    if Z.(succ curr_counter >= of_int i) then return (ctxt, curr_counter)
    else
      let*@ next = Alpha_context.Address_registry.next_counter_and_incr ctxt in
      loop next
  in
  loop (ctxt, Z.zero)
...
let test_init_context_counter () =
  ...
  let*@ _ctxt, first_available_counter =
...
let test_counter_increases ?(initial_counter = 10) () =
  let open Lwt_result_wrap_syntax in
  let* ctxt = init_context () in
  let* ctxt, initial_counter = init_counter ctxt initial_counter in
  let*@ _ctxt, next_counter =
    Alpha_context.Address_registry.next_counter_and_incr ctxt
  in
  if Z.(next_counter <> succ initial_counter) then
    failwith "Next counter should be the successor of the initial counter"
  else return_unit
