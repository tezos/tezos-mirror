...
(** Test [Slot_availability.intersection]

    Both parameters use the same indexing: [lag_index i] corresponds to [attestation_lags[i]]

    Example with attestation_lags = [1; 3; 5; 8]:
    - Both sa and att: lag_index 0 -> lag 1, lag_index 1 -> lag 3, etc. *)
let test_intersection_basic () =
  let open Lwt_result_wrap_syntax in
  let attestation_lags = [1; 3; 5; 8] in
  ...
  let commit_slots_sa sa ~lag_index slots =
    List.fold_left
      (fun sa slot ->
  ...
  let commit_slots_att att ~lag_index slots =
    List.fold_left
      (fun att slot ->
        Dal_attestations_repr.commit
          att
  ...
  let check_slots result ~lag_index expected_slots =
    let is_attested_result slot =
      Dal_attestations_repr.Slot_availability.is_attested
        result
        ~number_of_slots
        ~number_of_lags
        ~lag_index
  ...
  let sa = Dal_attestations_repr.Slot_availability.empty in
  let sa = commit_slots_sa sa ~lag_index:0 [0; 5] in
  let sa = commit_slots_sa sa ~lag_index:1 [1; 6] in
  ...
  let att = commit_slots_att att ~lag_index:2 [5; 10; 20] in
  let att = commit_slots_att att ~lag_index:3 [5; 20] in

  let result =
    Dal_attestations_repr.Slot_availability.intersection
      sa
      att
      ~number_of_slots
      ~attestation_lags
  ...
  let* () = check_slots result ~lag_index:0 [] in
...
(** Test intersection with empty inputs *)
let test_intersection_empty () =
  let open Lwt_result_wrap_syntax in
  let attestation_lags = [3; 5] in
  let number_of_lags = 2 in
  ...
  (* Build sa with slots at lag_index 0, but att has slots at lag_index 1 (no overlap) *)
  let sa = Dal_attestations_repr.Slot_availability.empty in
  let sa =
    Dal_attestations_repr.Slot_availability.commit
      sa
...
let tests =
  [
    ...
    Tztest.tztest
      "intersection basic (multiple lags)"
      `Quick
