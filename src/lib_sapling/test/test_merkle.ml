...
let test_merkle () =
  ...
  (* test a full tree *)
  let depth = 13 in
  let t = insert Empty hash1 depth 0L in
  let _w = get_witness_height t depth 0L in
  let rec loop t i =
    if i < 8192 (* = 2^13 *) then
      let t = insert t hash2 depth (Int64.of_int i) in
  ...
  (* let _w = get_witness_height t_final depth 8190L in *)
  ...
  let commitments =
    ...
    |]
  in
  let t = insert Empty commitments.(0) 4 0L in
  let rec loop i t =
    assert (get_root_height t depth = expected_merkle_root.(i)) ;
    if i < 15 then (
      let new_tree = insert t commitments.(i + 1) 4 (Int64.of_int (i + 1)) in
      assert (
...
let test_merkle2 () =
  ...
  in
  (* Compute the hash^i(uncomm) (ie. the default nodes at different heights) from the leaf up to one step before the root
...
let test_merkle3 () =
  ...
  let runs = 200L in
  assert (go_left 2L 1 = true) ;
  assert (go_left 2L 2 = false) ;
  ...
  let rec loop t pos =
    if pos > runs then ()
      ...
      let l = get_from t 0L |> TzOption.unopt_assert ~loc:__POS__ in
      assert (List.length l = Int64.to_int pos + 1) ;
      List.iter (fun e -> assert (e = cm)) l ;
      let l = get_from t pos |> TzOption.unopt_assert ~loc:__POS__ in
