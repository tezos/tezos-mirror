...
let test_scenario_t4 () =
  ...
  let b_endorsed_level_1 = ref false in
  ...
  let module Node_b_hooks : Hooks = struct
    ...
    let on_new_head ~block_hash ~block_header =
      ...
      | Some a_proposal_level_1
        when Block_hash.(a_proposal_level_1 = block_hash) ->
          Lwt.return_none
    ...
    let on_inject_operation ~op_hash ~op =
      let* is_endorsement = op_is_endorsement ~level:1l ~round:0l op_hash op in
      if is_endorsement then b_endorsed_level_1 := true ;
      return (op_hash, op, [Pass; Pass])

    let check_chain_on_success ~chain:_ =
      if not !b_endorsed_level_1 then
        failwith "Node B did not endorse proposal at level 1, round 0"
      else if
        not
        @@ Option.equal
             Block_hash.equal
