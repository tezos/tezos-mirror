...
(** [amplify shard_store node_store commitment node_ctxt] is called
    each time a new shard is received by an observer node, after being
    added to the shard store [shard_store]. The argument [commitment]
    is the commitment of the received shard. *)
val amplify :
  Store.Shards.t ->
  Store.node_store ->
