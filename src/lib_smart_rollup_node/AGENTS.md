...
Almost every function takes `Node_context.t`. It uses phantom types to encode **store** and **context** access permissions at compile time (see `lib_layer2_store/AGENTS.md` for the full permission system).
...
|-----------|-----------|------|
| `src/lib_smart_rollup/` | [AGENTS.md](../lib_smart_rollup/AGENTS.md) | Protocol-agnostic shared types (commitments, inbox, operations, RPC service definitions) |
| `src/bin_smart_rollup_node/` | [AGENTS.md](../bin_smart_rollup_node/AGENTS.md) | CLI binary entry point, protocol plugin linking |
| `src/lib_layer2_store/` | [AGENTS.md](../lib_layer2_store/AGENTS.md) | Storage abstraction: context signatures, phantom-typed access modes, context dispatch |
| `src/lib_layer2_irmin_context/` | [AGENTS.md](../lib_layer2_irmin_context/AGENTS.md) | Irmin context backend (WASM/Arith PVMs) |
| `src/lib_layer2_riscv_context/` | [AGENTS.md](../lib_layer2_riscv_context/AGENTS.md) | RISC-V context backend (Rust bindings) |
| `src/lib_injector/` | [AGENTS.md](../lib_injector/AGENTS.md) | Generic L1 operation injection framework (functor, lifecycle, retry) |
| `src/lib_sqlite/` | [AGENTS.md](../lib_sqlite/AGENTS.md) | Type-safe SQLite wrapper (Caqti, pooling, transactions) |
...
## Keeping AGENTS.md files up to date

When modifying the rollup node codebase, update the relevant AGENTS.md if your change affects documented patterns, adds new components/files worth mentioning, changes module opens, alters common task workflows, or introduces new anti-patterns. This applies to all 8 AGENTS.md files in related directories (see "Related directories" above).
